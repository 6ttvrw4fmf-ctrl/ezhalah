...
check('city onFocus with existing text runs the match immediately (cohort-typed)', /onFocus=\{\(\) => \{[\s\S]{0,2400}?\} else \{[\s\S]{0,700}?if \(!isLatinOnlyInput\(query\.location\)\) \{\s*setCitySuggestions\(matchCitiesByText\(effDeal, rentPeriodTok, effCategory, query\.location, cohortTypes\)\);/.test(indexSrc));
check('district onFocus with existing text runs the match immediately (cohort-typed)', /\} else if \(!isLatinOnlyInput\(districtTextRef\.current\)\) \{[\s\S]{0,300}?setDistrictSuggestions\(matchDistrictsByCityId\(citySelected\.cityId, effDeal, effCategory, rentPeriodTok, districtTextRef\.current, cohortTypes\)\);/.test(indexSrc));
