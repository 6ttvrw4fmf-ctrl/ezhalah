...
check('city onFocus with existing text runs the match immediately', /onFocus=\{\(\) => \{[\s\S]{0,2400}?\} else \{[\s\S]{0,700}?if \(!isLatinOnlyInput\(query\.location\)\) \{\s*setCitySuggestions\(matchCitiesByText\(query\.deal, paymentMonthly, effCategory, query\.location\)\);/.test(indexSrc));
check('district onFocus with existing text runs the match immediately', /\} else if \(!isLatinOnlyInput\(districtTextRef\.current\)\) \{[\s\S]{0,300}?setDistrictSuggestions\(matchDistrictsByCityId\(citySelected\.cityId, query\.deal, effCategory, paymentMonthly, districtTextRef\.current\)\);/.test(indexSrc));
