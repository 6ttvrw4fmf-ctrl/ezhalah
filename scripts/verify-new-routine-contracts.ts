...
for (const spec of [...specs, 'ENGINEER_ROUTINES.md', 'AUTONOMOUS_INCIDENT_LOOP.md']) {
  ...
  for (const m of readFileSync(p, 'utf8').match(PATH_RE) ?? []) {
    if (m.includes('*') || m === KNOWN_PHANTOM_CITATION) continue;
    if (!existsSync(__join(REPO_ROOT, m))) phantoms.push(`${spec} → ${m}`);
  }
}
...
mustCatch('a contract falling out of the roster',
  !roster.replace(/LISTING_LIFECYCLE_ENGINEER\.md/g, '').includes('LISTING_LIFECYCLE_ENGINEER.md'));
mustCatch('a spec naming a barrier that does not exist',
  !existsSync(__join(REPO_ROOT, 'scripts/verify-a-guard-nobody-ever-wrote.ts')));
