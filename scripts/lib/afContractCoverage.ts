...
const S14: Entry[] = [
  ...
  { rule: 'R14.2.1', dim: 'trending', weight: 3, grade: 'L', barrier: ['verify-district-counts-honest', 'verify-count-surfaces-share-the-results-purity-gate', 'verify-trending-pool-caches-recover'], evidence: 'browser desktop+mobile UI = REQUEST = RPC (الرياض 37,492 · جدة 22,365 · الدمام 6,988 · الخبر 5,715). 2026-09-05: the purity-gate barrier now reconciles EVERY discovered scope-carrying count surface against the results RPC under the identical scope, and covers canonical city clusters BY NAME rather than by top-N ranking — which is how the الهفوف panel defect (335 vs 579, ops_incident #32) had been escaping: it was sampled only while a separate 92% under-count pushed that city into the top 3. 2026-09-18: the cluster machinery behind those by-name counts is now executed against an injected FAILED loc_city_cluster read (ops_incident #268) — a failure used to be memoised as an empty map, which silently made applyClusterUnion a no-op and put الاحساء back to advertising 3,677 against a 4,953 click' },
