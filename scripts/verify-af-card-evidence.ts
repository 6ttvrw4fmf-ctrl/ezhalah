...
console.log('\n── T4. injection: one carrier, one prop, comparator, strip only from afEvidence ────────');
{
  ...
  const end = card.indexOf('testID="card-af-evidence-more"');
  assert(start > 0 && end > start, 'the strip renders card-af-evidence … card-af-evidence-more');
  const strip = card.slice(card.lastIndexOf('{evidence.length > 0 ?', start), end);
  ...
  assert(/\(afOpen \? evidence : evidence\.slice\(0, AF_CHIP_CAP\)\)\.map\(/.test(strip), 'chips are mapped from `evidence` with the AF_CHIP_CAP slice');
  assert(strip.includes('testID={`card-af-evidence-${c.id}`}'), 'each chip carries card-af-evidence-{id}');
  assert(card.includes('+{evidence.length - AF_CHIP_CAP}'), 'the overflow chip prints +N over the cap');
  assert(card.includes('export const AF_CHIP_CAP = 4;'), 'AF_CHIP_CAP = 4 (owner decision 3)');
