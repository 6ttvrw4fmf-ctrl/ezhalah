...
//
// THE FAILURE CLASS THIS EXISTS FOR (found by the 2026-08-12 Search & Matching QA run):
...
//   the user sees the same physical property twice. Confirmed at introduction: dealapp 5 URL pairs,
//   sadin 5 URL pairs, 20 production_ready rows total.
//
// The barrier lives in supabase/migrations/20260812120526_url_collision_res_vs_com_detector.sql.
// It must stay in the roster (else mon_detect_orphaned_detectors would flag it),
// keep filtering to production_ready on BOTH sides (a non-served row is not a user-visible defect),
// self-heal via mon_resolve_key when a platform recovers, and never widen scope silently.
//
// Deliberately OFFLINE (tracked repo files only), like the other verifiers in `npm test`.
// The LIVE half is mon_detect_orphaned_detectors(), which raises P2 within seconds if the
// detector ever becomes unreachable from the roster.
//
...
if (creators.length) {
  ...
  check(sql.includes('pg_get_functiondef'),
    ...
    `detector the author's copy predates (recorded losses; see verify-detector-roster-edits-are-guarded.ts)`);
