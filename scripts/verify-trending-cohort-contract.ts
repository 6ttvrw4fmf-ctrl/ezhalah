...
check('district cache key includes the cohort types', /districtCacheKey = \([^)]*types[^)]*\)[^`]*`\$\{cityId\}:\$\{deal\}:\$\{category \?\? ''\}:\$\{pmKey\(periodTok\)\}:\$\{typesKey\(types\)\}:\$\{afKey\(scope\)\}`/.test(loc));
...
check('city + district effects re-run when the cohort types change',
  // Trailing deps are open-ended on purpose: the city effect gained cityAfSig (2026-08-22) and the
  // district effects gained cityTableScopeSig (2026-09-03). The invariant is that cohortTypesSig is
  // IN the dep list, not that it is the last entry — enumerating the known siblings just guarantees
  // this check breaks again the next time a real dependency is added.
  (idx.match(/cohortTypesSig(?:, \w+)*\]\);/g) || []).length >= 3);
