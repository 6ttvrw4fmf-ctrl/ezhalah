...
check('rehydration reads the same canonical catalog the field picks from (ensureDistrictOptions)',
  /ensureDistrictOptions\(\s*citySelected\.cityId,\s*effDeal,\s*effCategory,\s*rentPeriodTok,\s*cohortTypes\s*\)/.test(effect));
...
check('effect re-runs on city/deal/category/period change',
  /\}, \[citySelected, effDeal, effCategory, rentPeriodTok, cohortTypesSig\]\);/.test(effect));
