...
check('onFocus with empty text shows the deal+period+category-scoped Top 6 (topCitiesByListings(query.deal, paymentMonthly, effCategory, 6))', /onFocus=\{\(\) => \{[\s\S]{0,1800}?topCitiesByListings\(query\.deal, paymentMonthly, effCategory, 6, cohortTypes\)/.test(indexSrc));
check(
  ...
  /if \(!cityTextRef\.current\) setCitySuggestions\(topCitiesByListings\(query\.deal, paymentMonthly, effCategory, 6, cohortTypes\)\);/.test(indexSrc) && /cityTextRef\.current = v;/.test(indexSrc),
);
check(
  ...
  /void ensureCityFieldIndex\(query\.deal, paymentMonthly, effCategory, cohortTypes\)\.then\(\(pool\) => \{[\s\S]{0,700}?if \(cityTextRef\.current\) \{[\s\S]{0,200}?setCitySuggestions\(latin \? \[\] : matchCitiesByText\(query\.deal, paymentMonthly, effCategory, cityTextRef\.current, cohortTypes\)\);/.test(indexSrc),
);
check(
  ...
  /\}, \[query\.deal, paymentMonthly, effCategory, cohortTypesSig\]\);/.test(indexSrc) && /else if \(cityFocus\) \{\s*setCitySuggestions\(topCitiesByListings\(query\.deal, paymentMonthly, effCategory, 6, cohortTypes\)\);/.test(indexSrc),
);
