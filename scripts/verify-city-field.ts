...
// 2026-07-21 (PR#167/#175, LIVE) to also thread rentPeriodTok (Rent's Monthly/Yearly toggle;
...
check('onFocus with empty text shows the deal+period+category-scoped Top 6 (topCitiesByListings(query.deal, rentPeriodTok, effCategory, 6))', /onFocus=\{\(\) => \{[\s\S]{0,1800}?topCitiesByListings\(query\.deal, rentPeriodTok, effCategory, 6, cohortTypes\)/.test(indexSrc));
check(
  ...
  /if \(!cityTextRef\.current\) setCitySuggestions\(topCitiesByListings\(query\.deal, rentPeriodTok, effCategory, 6, cohortTypes\)\);/.test(indexSrc) && /cityTextRef\.current = v;/.test(indexSrc),
);
check(
  ...
  /void ensureCityFieldIndex\(query\.deal, rentPeriodTok, effCategory, cohortTypes\)\.then\(\(pool\) => \{[\s\S]{0,700}?if \(cityTextRef\.current\) \{[\s\S]{0,200}?setCitySuggestions\(latin \? \[\] : matchCitiesByText\(query\.deal, rentPeriodTok, effCategory, cityTextRef\.current, cohortTypes\)\);/.test(indexSrc),
);
check(
  ...
  /\}, \[query\.deal, rentPeriodTok, effCategory, cohortTypesSig\]\);/.test(indexSrc) && /else if \(cityFocus\) \{\s*setCitySuggestions\(topCitiesByListings\(query\.deal, rentPeriodTok, effCategory, 6, cohortTypes\)\);/.test(indexSrc),
);
