...
// Signature of the SHIPPED renderer on main: (name, cond, detail) — name first, and exported.
type Render = (name: string, cond: boolean, detail: string) => string;
...
const { renderLine } = (await liftSymbols(
  ...
  [{ header: 'export const renderLine =', endsWith: /;\s*$/ }],
  ['renderLine'],
...
const leaksOnPass = (r: Render) => r('a check', true, FAIL_TEXT).includes(FAIL_TEXT);

check('a PASS line carries the name only, never the failure explanation',
  !leaksOnPass(renderLine), renderLine('a check', true, FAIL_TEXT));
check('a FAIL line still carries it (the fix is not «delete detail», which would blind failures)',
  renderLine('a check', false, FAIL_TEXT).includes(FAIL_TEXT), renderLine('a check', false, FAIL_TEXT));
check('the tick and the cross themselves are unchanged',
  renderLine('a check', true, '') === '  ✓ a check' && renderLine('a check', false, '') === '  ❌ a check',
  `${renderLine('a check', true, '')} / ${renderLine('a check', false, '')}`);

// MUTATION, executed: incident #42's renderer, verbatim as it stood on 2026-09-05.
const legacy: Render = (name, cond, detail) => `  ${cond ? '✓' : '❌'} ${name}${detail ? ` — ${detail}` : ''}`;
