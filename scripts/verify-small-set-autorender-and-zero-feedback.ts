...
check("the local wrapper feeds it quotableTotal (the honest total) and INTERVIEW_STOP_AT",
  ...
  /initialRevealPure\(\{ fetched: r\?\.listings\?\.length \?\? 0, honestTotal: r \? quotableTotal\(r\) : null, firstPage: FIRST_PAGE, stopAt: INTERVIEW_STOP_AT, platforms: distinctPlatformCount\(r\?\.listings\), afCompleted \}\)/.test(agent));
