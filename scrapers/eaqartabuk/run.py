...
# District title-fallback (2026-08-04 audit): candles-map's own `district` field is blank on
# ~21% of listings (115/546), but the WordPress list `title` frequently embeds it — e.g.
# "شقة للبيع في تبوك (حي حي البوادي)" or "فيلا روف ارضي بحى الصفا" (live-verified against
# eaqartabuk.com). Mirrors alkhaas's proven بحي/حي title-scrape (alkhaas/run.py map_listing),
# widened to also match the alef-maqsura spelling «حى». Captures ONE word only (re-verified via
# a dry-run resolve_district_ar() pass against every null-district row: a single-word capture
# resolved 72/115 rows vs only 46/115 for a two-word capture, because eaqartabuk titles often
# trail the district with room counts / "|" separators / phrases that a 2-word window swallows
# and breaks the catalog match on). The `(?<=\s)` boundary also handles the doubled "(حي حي X)"
# wrapper for free: the first حي sits right against "(" with no space, so the boundary skips it
# and matches the second حي, capturing just "X". No prefix massaging needed — resolve_district_ar
# normalizes away a leading حي on both sides before comparing, so "الصفا" and "حي الصفا" resolve
# identically; an unresolvable capture just fails the catalog match and stays NULL as it did
# before this fallback existed, so a bad capture can never produce a wrong match, only a miss.
_TITLE_DISTRICT_RE = re.compile(r"(?:^|(?<=\s))ب?ح[يى]\s+([^\s,،.)\]]+)")


def _district_from_title(title: Optional[str]) -> Optional[str]:
    m = _TITLE_DISTRICT_RE.search(title or "")
...
def map_listing(item: dict, mp: dict, desc_html: Optional[str],
    ...
    if not district and raw_city and raw_city not in ("تبوك", "Tabuk"):
        ...
        district = raw_city
    if not district:
        district = _district_from_title(item.get("title"))
...
def main() -> int:
    ...
    try:
        ...
        superseded = db.retire_superseded_siblings(
            res_table="eaqartabuk_residential_listings", com_table="eaqartabuk_commercial_listings",
        ...
        healthy = db.end_run(run_id, ok=True, rows_seen=seen, rows_upserted=seen, notes=f"pruned={pruned}", check_tables=["eaqartabuk_residential_listings", "eaqartabuk_commercial_listings"])
        if not healthy:
