"""Abwbna scraper — SAME PLATFORM AS ALDARIM, a different Nuzul SaaS tenant.

Found 2026-09-06 while investigating why the site's homepage TLS-failed on curl but its cover
images resolved to nuzul-saas-production.s3.us-east-2.amazonaws.com/tenants/3846/... — the EXACT
S3 bucket family aldarim's tenant (3567) uses. Confirmed by guessing the sibling API host from
aldarim's own pattern (<slug>.nzl-backend.com/api/public/properties) — it answered on the first
try, no auth, same JSON shape byte-for-byte (category/purpose/type/city{name_en,name_ar}/
selling_price/images[]/cover_image_url/availability_status…). 215 listings across 3 pages.

This file is therefore a close clone of scrapers/aldarim/run.py, not a fresh reverse-engineering —
every tri-state flag helper, the kitchen/age resolvers, and the photo/additional_info extraction
solve the SAME trap on this tenant's data, because it is the same platform. Only the endpoint,
source name, and ad-number prefix differ. If a shared helper changes for aldarim, check whether
the same defect exists here before assuming it does not.

One difference verified live: abwbna's availability_status also has 'reserved' (1 of 100 sampled),
not just 'available'/'sold'. Handled the same way aldarim handles anything non-'available': not
ingested this run, then caught by the full-refresh prune below if it stays gone.

One type this tenant uses that aldarim never has: 'station' (1 of 100 sampled). normalize.map_type_en
has no answer for it — it falls through to None and the existing `if not row.get("property_type")`
guard drops it, exactly like any other unmapped type. Not special-cased; the guard already covers it.
...
Usage:  python -m scrapers.abwbna.run --pages 50 [--limit-test 1]
"""
...
API = "https://abwbna.nzl-backend.com/api/public/properties"
SITE = "https://www.abwbna.com/properties"
HEADERS = {"Accept": "application/json", "Origin": "https://www.abwbna.com",
           "Referer": "https://www.abwbna.com/"}
...
def map_listing(L: dict) -> tuple[Optional[dict], str]:
    ...
    row = {
        "ad_number": f"ABW{pid}",
        "listing_url": f"https://www.abwbna.com/properties/{pid}",
        "source": "Abwbna",
        "active": True,
...
def main() -> int:
    ...
    s = session()
    run_id = None if args.limit_test else db.begin_run("abwbna")
    res_rows: list[dict] = []
    com_rows: list[dict] = []
    _, last_page = fetch_page(s, 1)
    pages = min(args.pages, last_page)
    print(f"Abwbna: {last_page} pages total, scraping {pages} (per_page={PER_PAGE})")
    seen = 0
    try:
        ...
        if args.limit_test:
            ...
            return 0
        if res_rows:
            db.upsert_abwbna_residential_batch(res_rows)
        if com_rows:
            db.upsert_abwbna_commercial_batch(com_rows)
        ...
        if not args.pages or pages >= last_page:  # only prune on a FULL crawl, never a partial run
            ...
            superseded = db.retire_superseded_siblings(
                res_table="abwbna_residential_listings", com_table="abwbna_commercial_listings",
                res_ads=set(seen_res), com_ads=set(seen_com),
                source="Abwbna")
            if superseded:
                print(f"  retired {superseded} superseded sibling row(s) after a category flip")

            for tbl, seen_ads in (("abwbna_residential_listings", seen_res), ("abwbna_commercial_listings", seen_com)):
                n = db.prune_unseen(tbl, set(seen_ads), source="Abwbna")
                if n < 0:
                    print(f"⚠ {tbl}: prune guard tripped (0 scraped or collapse) — kept existing active")
                else:
                    pruned += n
        print(f"✓ Abwbna: {len(res_rows)} residential + {len(com_rows)} commercial upserted, {pruned} stale pruned")
        healthy = db.end_run(run_id, ok=True, rows_seen=seen, rows_upserted=len(res_rows) + len(com_rows), notes=f"pruned={pruned}", check_tables=["abwbna_residential_listings", "abwbna_commercial_listings"])
        if not healthy:
