...
def _pin_sold_inactive(table: str, ad_numbers: list[str],
    ...
    exactly what let sold listings resurrect every morning (3 aqaratikom_commercial rows sat in
    that vulnerable state on 2026-07-16). So AFTER the batch upsert we pin the sold rows to
    missing_count=3 (the existing prune 3-strike threshold) + active=false.
    ...
    sold_pin.pin_source_confirmed_gone(
        table, ad_numbers, oracle="aqaratikom.sold_pin.is_sold", seen_ad_numbers=seen_ad_numbers,
    )
...
def main() -> int:
    ...
    try:
        ...
        superseded = db.retire_superseded_siblings(
            res_table="aqaratikom_residential_listings", com_table="aqaratikom_commercial_listings",
