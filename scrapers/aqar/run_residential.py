...
def main() -> int:
    ...
    try:
        ...
        # run_commercial.py, which used `ok=ok` (a variable, not the `ok=True` literal
    ...
    # Exit 0 only when the run COMPLETED cleanly AND end_run() did not demote it. A small
    # town/type-group with zero new inventory is a valid result, not a failure — but a demotion
    # (0-row/dead-source, tripped floor, degraded integrity check) must redden CI.
