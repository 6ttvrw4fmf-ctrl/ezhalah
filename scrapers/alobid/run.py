"""Alobid scraper -- a Nuzul SaaS tenant (same platform as aldarim/abwbna).

Found 2026-09-06 as a fifth Nuzul SaaS tenant -- same discovery method as bahadhabab (the
'nzl-backend.com' subdomain is 'alobidoffice', matching the site's own domain exactly this
time). Same public JSON API, no auth, byte-identical shape. 139 listings across 2 pages, all
100 sampled were availability_status='available' -- the cleanest of the Nuzul tenants seen so
far, though the existing not-in-('available','','None') guard still applies unchanged.
...
API = "https://alobidoffice.nzl-backend.com/api/public/properties"
SITE = "https://www.alobidoffice.com/properties"
HEADERS = {"Accept": "application/json", "Origin": "https://www.alobidoffice.com",
           "Referer": "https://www.alobidoffice.com/"}
...
def map_listing(L: dict) -> tuple[Optional[dict], str]:
    ...
    row = {
        "ad_number": f"ALB{pid}",
        "listing_url": f"https://www.alobidoffice.com/properties/{pid}",
        "source": "Alobid",
        "active": True,
...
def main() -> int:
    ...
    s = session()
    run_id = None if args.limit_test else db.begin_run("alobid")
    res_rows: list[dict] = []
    com_rows: list[dict] = []
    _, last_page = fetch_page(s, 1)
    pages = min(args.pages, last_page)
    print(f"Alobid: {last_page} pages total, scraping {pages} (per_page={PER_PAGE})")
    seen = 0
    try:
        ...
        if args.limit_test:
            ...
            return 0
        if res_rows:
            db.upsert_alobid_residential_batch(res_rows)
        if com_rows:
            db.upsert_alobid_commercial_batch(com_rows)
        ...
        if not args.pages or pages >= last_page:  # only prune on a FULL crawl, never a partial run
            ...
            superseded = db.retire_superseded_siblings(
                res_table="alobid_residential_listings", com_table="alobid_commercial_listings",
                res_ads=set(seen_res), com_ads=set(seen_com),
                source="Alobid")
            if superseded:
                print(f"  retired {superseded} superseded sibling row(s) after a category flip")

            for tbl, seen_ads in (("alobid_residential_listings", seen_res), ("alobid_commercial_listings", seen_com)):
                n = db.prune_unseen(tbl, set(seen_ads), source="Alobid")
                if n < 0:
                    print(f"⚠ {tbl}: prune guard tripped (0 scraped or collapse) — kept existing active")
                else:
                    pruned += n
        print(f"✓ Alobid: {len(res_rows)} residential + {len(com_rows)} commercial upserted, {pruned} stale pruned")
        healthy = db.end_run(run_id, ok=True, rows_seen=seen, rows_upserted=len(res_rows) + len(com_rows), notes=f"pruned={pruned}", check_tables=["alobid_residential_listings", "alobid_commercial_listings"])
        if not healthy:
