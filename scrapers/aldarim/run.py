...
def map_listing(L: dict) -> tuple[Optional[dict], str]:
    ...
    # measure_num, not _int: the API's 407.56 stays 407.56 (int(float()) truncated it). 0 = "not set".
...
def main() -> int:
    ...
    try:
        ...
        healthy = db.end_run(run_id, ok=True, rows_seen=seen, rows_upserted=len(res_rows) + len(com_rows), notes=f"pruned={pruned}", check_tables=["aldarim_residential_listings", "aldarim_commercial_listings"])
        if not healthy:
