...
/**
 * The listing FETCH the results screen ran — the SUBMITTED search, never one of the `p_limit: 1`
 * option-count probes that ride the same RPC name (§11.3, isResultsSearch()). This used to re-derive
 * the classification inline as `p_limit > 100`; the harness now classifies every request as it
 * arrives, and one taxonomy in one place is what stopped ops_incident #51 (a Filter home that had
 * submitted nothing was accused of firing a search because an option count was counted as one).
 */
const listingFetch = (ctx) => ctx.resultsSearches[ctx.resultsSearches.length - 1] ?? null;
