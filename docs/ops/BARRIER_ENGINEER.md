...
run, `barriers: N · grandfathered: N · held to the rule: N`. Measured 2026-09-14 it reads
**524 · 305 · 219** (492 · 310 · 182 on 2026-09-12; 363 · 333 · 30 when this spec was written, and
362 · 333 · 29 in the audit that authorised the routine). **Read the printed line every run; never
quote a number from prose, including this paragraph** — the refresh above is a courtesy to the next reader, not a source.
...
`required run set · live-reaching candidates · production-dependent (ceiling)`, which on 2026-09-14
reads **464 · 9 · 4 (ceiling 4)**. `production-dependent` is shrink-only and every member is a check
