...
> Owner rule: there are **exactly SEVEN separate cloud routines** (the fourth added by the owner
> 2026-08-11; the fifth added 2026-08-23; the sixth and seventh added 2026-08-26). Six run DAILY;
> the seventh (#7, 🧵) is deliberately scheduled for the evening rather than the shared morning
> block — see "Schedule note" below. They are never merged, renamed into each other, or
...
| 6 | 👣 Daily Journey & Persistence Engineer | *(pending — owner creates in routines UI)* | 06:00 | 13:00 | claude-opus-5 | Real-user journeys: state, navigation, sessions, sidebar/history/Favorites, cross-browser/device — never search matching itself |
| 7 | 🧵 Daily Systems Seam Engineer | *(pending — owner creates in routines UI)* | 18:00 | 01:00 (+1) | claude-opus-5 | Cross-system integration integrity: cron→detector→alert, migration→mirror→prod, deploy-claim-vs-served-bundle, RLS, orphaned guarantees |
...
**Schedule note (2026-08-26):** routine #6 (👣) continues the existing 30-minute stagger, landing
at 06:00 Arizona — 30 minutes after #4 and reading #4/#5's freshest reports on the way in, since
its journeys pass through Normal Filter and AF on the way to testing what's *around* them. Routine
#7 (🧵) is deliberately placed OUTSIDE the morning block, at 18:00 Arizona: the seams it audits
(did today's crons fire, did today's deploy actually ship, is a lock stuck) are questions about a
day that hasn't happened yet at 06:00. Running it in the evening, after a full day of production
traffic, cron activity, and any deploys, gives it something real to find. This also means #7 rarely
contends with the other six for the deploy lock — a useful side effect, not the reason for the
placement.
...
never reorder the original four without also re-checking that dependency. Routines #6 and #7 read
the freshest reports from whichever of #1–#5 owns the surface they're about to touch, but do not
sit inside that same load-bearing chain — #6 runs later the same morning, #7 runs that evening, and
neither routine's own output is consumed by an earlier one the next day. Routines are managed at
...
migration). Deliberately scheduled for the evening (18:00 Arizona) rather than the shared morning
block — see "Schedule note (2026-08-26)" above.
