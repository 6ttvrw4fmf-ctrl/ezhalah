...
| Field | Value |
|---|---|
| Date | 2026-07-27 (audit items 1–7: hierarchy unification, restore sanitizer, price-flip clarity, fidelity + counts-RPC parity — see PR #247/#248) — CURRENT LIVE |
| Vercel deployment ID | `dpl_5XGrpw6FLcEUWNYm1ge6rUsFgmY9` |
| Production URL | `https://ezhalah-app.vercel.app` |
| Bundle hash | `entry-e98326bb1d0079da92ceaa310a80c820.js` |
| Deployed from | `main` @ `67a4f1d` (PR #247 squash-merged as `f3db8d4`, PR #248 drift recovery), via `scripts/safe-deploy.sh` (MCP-held-lock mode) from a clean checkout. |
| Contains | Audit items 1–7 (owner-approved 2026-07-27): interview unified onto `propertyTypes.ts` HIERARCHY (typeGroup step, CATEGORY_TYPES retired); `sanitizeForFilterRestore` strict allowlist for history restore; monthly↔yearly price flip clears + Arabic note; mustqr ppm derivation removed (648 cleared, migration `20260727153645`); refine-flow bedroom gate on canonical applicability; `known_type_ar` seed emits «غير معروف» sentinel + live==repo canon-sync test; counts-RPC hardening (`20260727155517`: Buy period bypass, zero=unbounded, empty-array guards, both-macro gate); `sql/mirrors/` for live-only objects; Abha catalog label → «أبها» (`20260727175041`). 5 new permanent suites in `npm test`. Full detail: memory `project_audit-items-1-7-executed-2026-07-27`. |
| Verified post-deploy | Live bundle `entry-e98326bb…` served on `ezhalah-app.vercel.app`: Supabase ref present, `sanitizeForFilterRestore` ×2 present, both new i18n entries present (Arabic stored as `\uXXXX` with literal spaces). Real browser click-paths on production: monthly→yearly flip cleared both price inputs + rendered «تم مسح حدود السعر…»; typing «ابها» suggests «أبها» and searching returned 198 rent listings with correct summary (سكني/سنوي/أبها/عسير) and rendered cards. Counts-RPC parity via anon REST: guided Buy+فيلا+شهري 26,707 == search; `p_price_max=0` 10,013 == unbounded; standard parity guided==search==age 10,013; both-macro «غير معروف» 15/15 Residential + 0/0 Commercial. `npm test` 625 PASS incl. 5 new suites; `tsc` clean; drift gate green (2 parallel-session migrations recovered byte-exact, md5-proven, PR #248). |
