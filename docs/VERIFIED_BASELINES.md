...
| aqar's `Search.find` pages **without a stable tiebreaker**: one `from`/`size` pass returns the same id on two pages and never shows others, losing ~19% of the declared total (measured: 194-199 distinct against a declared 240). Re-paging converges (194 → 224 → 234 → 239). A page is capped at ~70 rows however large a `size` is asked for, so a bigger page cannot substitute. | 2026-09-12 | drive `scrapers/aqarmonthly/run.py::discover_ids` and compare `len(d.ids)` to `d.declared` | aqar adds a stable sort/`search_after`, or the ~70-row page cap moves |
| The **aqar DailyRenting `availability:{eq:1}` facet collapses every Saturday** — five consecutive (2026-08-15/22/29, 09-05, 09-12) at ~240 vs ~3,736 on other days — while the vertical itself stays whole (unfiltered total 3,953; `availability:{eq:0}` 399; the remaining ~3,300 carry no availability value at all). It is the SOURCE's own answer, recovers the next day, and must not be read as a partial crawl. | 2026-09-12 | `select started_at::date, to_char(started_at,'Dy'), min(rows_seen), max(rows_seen) from scrape_runs where platform='aqarmonthly' and started_at > now()-interval '35 days' group by 1,2 order by 1 desc;` | A Saturday stops collapsing, or the collapse spreads to other weekdays |
