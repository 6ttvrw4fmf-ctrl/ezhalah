...
9. **An unresolved‑location listing is scoped to Saudi Arabia (country‑wide) until it resolves — not hidden.** (User rule, 2026‑06‑28.)
   ...
   - _Mechanism:_ the three read RPCs (`location_search_candidates_ar`, `apartment_guided_counts_ar`, `property_age_option_counts_ar`) include a row that is **genuinely unlocated** (`region_id IS NULL OR city_id IS NULL`) when **no** location scope is given (`p_cities`/`p_districts`/`p_region_ids` all NULL), and require `production_ready` once any scope is set — so this rule is **already live for the review queue**. _Open:_ ~1,706 active listings are absent from the search view (`listing_native_location_v2`) entirely (resolution fully failed + not in the legacy fallback), so they are invisible even country‑wide; a catch‑all that lets them through with NULL location is **pending explicit approval** (it edits the resolution view).
   - ⚠️ **`production_ready` is NOT a pure location gate** (corrected 2026‑08‑10 — the earlier wording here implied it was). It is set by `listing_native_location_v2` as `region_id IS NOT NULL AND city_id IS NOT NULL`, and then the `BEFORE INSERT OR UPDATE` trigger `enforce_price_size_sanity()` **clears it** for any row where `price_size_impossible(price_total, price_annual, area_m2)` holds (the PR#298 safety gate). So `production_ready = false` means _either_ "location unresolved" _or_ "value withheld as impossible".
   - Because of that, the country‑wide fallback must test **the row being unlocated**, not merely "the user supplied no location filter". Until 2026‑08‑10 it tested the latter, so every located‑but‑withheld row was reachable in an unfiltered search and vanished the moment a city was picked — e.g. `dealapp/5696027` in جدة at 22,002,786,078,000 SAR over 564,174 m². Migration `20260810123000` added the `and (s.region_id is null or s.city_id is null)` guard to all three RPCs. Barrier: view `mon_located_row_visible_only_unfiltered` + `scripts/verify-unlocated-fallback-scope-live.ts`.
