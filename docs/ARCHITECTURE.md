...
- **First-100 diversity — four dimensions, in this priority order (owner PERMANENT rule, 2026-09-14).** The first «عرض المزيد» batch (up to 100 shown) must feel curated, not dumped, across the four dimensions below. This applies to Buy searches, Rent searches, and combined searches. Every engineer changing the order code (`src/lib/platformDiversity.ts::orderByScope` and its caller in `src/data/remote.ts`) is expected to read this rule first.
  ...
  4. **Photos** — a leaf preference, not a filter. Within an otherwise-identical group (same platform, same type, same period, same deal), listings that carry photos come first; no-photo listings are still shown, just later. Never remove a no-photo listing — MATCH FIRST binds.
  Every dimension is a permutation of the already-matched set — it can never add a listing the match did not produce. Barriers: `scripts/verify-first-batch-four-dimension-diversity.ts` executes the real `orderByScope` and mutation-proves each dimension; `scripts/verify-platform-diversity.ts` continues to pin the platform half; `scripts/verify-match-first-stages-are-order-only.ts` proves the whole stage is a pure permutation.
