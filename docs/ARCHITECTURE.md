...
| **AI agent** | Supabase Edge Function named **`agent`**. `AGENT_PROVIDER` selects the model — `gemini` (default; 2.5 Flash primary + Flash-Lite fallback via `GEMINI_MODEL`/`GEMINI_FALLBACK_MODEL`) or `deepseek` (added 2026-08-25, via `DEEPSEEK_API_KEY`/`DEEPSEEK_MODEL`, default `deepseek-v4-flash`) — both run the identical SYSTEM prompt + deterministic post-model pipeline (price extraction, the location catalog backstop, language guards); only the model-call shape differs. Barrier: `scripts/verify-agent-provider-switch.ts`. Runtime behavior tunable via the `agent_notes` DB table (no redeploy), independent of provider. |
