        ...
        >
          <View style={s.col}>
            {(() => { const lastId = msgs[msgs.length - 1]?.id; return msgs.map((m) => {
              ...
              // «مطابق لطلبك»: the ONLY carrier of the predicates these cards were searched with is this
              // turn's frozen result.query — never the Filter store, guidedPills or afReceipt. afActive()
              // is WeakMap-memoised on that object, so every card of the turn (and every «عرض المزيد»
