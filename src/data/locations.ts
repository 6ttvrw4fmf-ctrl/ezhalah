...
// CITY_TOKENS moved to src/lib/cityDisplay.ts (zero-dependency, so it can be executed by a real
// runtime test) — imported above.
...
// CITY_AR_DISPLAY (tier 1, hand-maintained DB-label overrides) and the CITY_TOKENS reverse lookup
// (tier 3) moved to src/lib/cityDisplay.ts (zero-dependency, so real runtime tests can execute them —
// imported above). Tier 2 (the sa-locations.json catalog, CITIES_IDX) stays here.
export function cityDisplay(cityEn: string, locale: string): string {
  ...
  const tokenHit = cityTokensReverseLookup(cityEn);
  if (tokenHit) return tokenHit;
  // All three tiers missed — this can genuinely happen for a city that lands in the DB (e.g. via a
  // new scraper source, or a data-quality slip like a region name leaking into the city column) before
  // anyone adds it here. Self-guard so EVERY caller is protected automatically, not just the ones that
  // remember to wrap the result in arabicOrUnresolved() themselves (2026-07-13 sibling-leak audit —
  // src/data/search.ts's "Cities"/"Nearby Cities" summary lines called this unwrapped). A generic
  // Arabic placeholder beats leaking raw English into an otherwise-Arabic sentence.
  return arabicOrPlaceholder(cityEn, locale, LOCATION_UNRESOLVED_AR);
}
...
// already picked (Deal is chosen BEFORE City in the form, so it's always known here). Category
// (Residential/Commercial) is picked AFTER City/District in the current form order, so it is NOT
...
const districtCacheKey = (cityId: number, deal: Deal, category: Category | null, paymentMonthly: boolean | null) => `${cityId}:${deal}:${category ?? ''}:${pmKey(paymentMonthly)}`;

// Load (once, cached per city+deal+category) the full district catalog for a city_id. Never falls
// back to another city.
export async function ensureDistrictOptions(cityId: number, deal: Deal, category: Category | null, paymentMonthly: boolean | null = null): Promise<DistrictOption[]> {
...
export function topDistrictsForCityId(cityId: number, deal: Deal, category: Category | null, paymentMonthly: boolean | null = null, k = 6): DistrictOption[] {
  return (_districtCache.get(districtCacheKey(cityId, deal, category, paymentMonthly)) ?? []).filter((d) => d.listingCount > 0).slice(0, k);
}
...
export function matchDistrictsByCityId(cityId: number, deal: Deal, category: Category | null, paymentMonthly: boolean | null, query: string): DistrictOption[] {
  const all = _districtCache.get(districtCacheKey(cityId, deal, category, paymentMonthly)) ?? [];
  const q = norm(query);
