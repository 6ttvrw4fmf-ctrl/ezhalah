...
//
// R12A.5 — THE VOCABULARY MUST COVER THE CERTIFIED VOCABULARY. "Every certified amenity token must
// be renderable; a token the RPC can filter on but the card cannot draw is a defect of this rule,
// not a missing nice-to-have." So this map is COMPLETE against AMENITY_COL, and
// verify-af-card-evidence.ts fails if it ever is not — a token the filter accepts but this map does
// not name would pass ok() and then render nothing, which is precisely the silent hole §12A exists
// to close. The eight below were certified for chat on 2026-08-31 and given chips + counts by
// 20260902220000; their labelKeys are the ones AMENITIES_QUESTION itself uses, copied verbatim.
export const AMENITY_LABEL: Record<string, string> = {
  ...
  gym: 'Gym', pool: 'Pool', garden: 'Garden', balcony: 'Balcony',
  laundry_room: 'Laundry room', optical_fibers: 'Optical fibers',
...
export const AF_EVIDENCE: Record<string, EvidenceDef> = {
  ...
  amenities: {
    ...
    // token in AMENITY_COL lacks a label — so in shipped code this lookup never misses. The `?? null`
    // is the fail-SAFE, not the rule: a module-scope throw here would crash the whole results screen
    // for real users over a data mistake (verify-no-public-test-crash-surface.ts forbids exactly
    // that), and printing the raw English token to an Arabic reader would be worse than silence.
    chips: (k, _r, t) => k.map((x) => (AMENITY_LABEL[x] ? t(AMENITY_LABEL[x]) : null)).filter((x): x is string => x != null),
  },
  bathrooms: {
    fields: ['bathMin'],
