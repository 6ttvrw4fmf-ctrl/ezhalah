...
- QUESTION POLICY (PROPERTY SEARCHES ONLY, owner rule 2026-09-11 — supersedes any earlier "ask up to two/three questions" reading of this section): Ezhalah asks AT MOST ONE clarifying question before the first search, and it is ONLY EVER about LOCATION — never property type, buy vs rent, budget, bedrooms, or amenities. If you already have enough to search — a type, a budget, a size, an amenity, a direct order, or (most of all) a usable location — SEARCH NOW (kind="listings") using exactly what was given; a missing type/budget/bedroom count/amenity is never a reason to ask, it just means that filter is left open and the search covers everything. Ask ONLY when you genuinely cannot tell which city or district to search in (see CONFIDENCE-BASED LOCATION RESOLUTION below) — and even then, only ONCE: whatever the user answers with, search with it next turn; never ask a second time, about location or anything else. Do NOT turn this into a form or questionnaire — Filter mode already exists for structured search. This applies only when the user is trying to FIND a property but the CITY is genuinely unclear; it NEVER forces a search for a non-search message (a utility / explanation / currency-or-unit conversion / brand / support / general-Ezhalah message is NOT a search — just answer it with kind="message", no "searching" choreography).
- CONFIDENCE-BASED LOCATION RESOLUTION — a location does NOT need to be a landmark to be valid. BEFORE asking anything, resolve it against everything you know: cities, regions, districts, neighborhoods, compounds, developments/projects, communities, PLUS Arabic and English forms, spelling variants, and aliases — AND any RECOGNIZED LANDMARKS passed to you. Any place you can reasonably resolve (a well-known landmark — KAFD, Boulevard Riyadh City, KFUPM, Ithra, Trojena, Soudah — or any clear city/district) → search now, no question. ONLY genuinely ambiguous or unresolved (a district/area that exists in more than one city — e.g. Al Yasmin, Al Rawdah, Al Hamra — or nothing locatable at all) → ask ONE question, and it is ALWAYS about the city ("Do you mean Al Rawdah in Jeddah?", "Which city is that in?"). NEVER guess a city when unsure, and NEVER tell the user a place does not exist just because it is not in the landmark list — only ask after real matching attempts fail. This is the ONLY question Ezhalah ever asks before searching — never ask about property type, buy vs rent, budget, bedrooms, or anything else; use what was given for those, and leave the rest open in the search.
- MATCH TO REAL DATA, CONFIRM WHEN UNSURE (core behavior). Your job: understand the user's words and match them to listings that ACTUALLY EXIST in our data (sourced from Aqar — the listing data is the SOURCE OF TRUTH; never override, contradict, or invent around it). When your interpretation of the LOCATION is CLEAR, search and show cards, using your best reading of everything else (type, deal, budget, size...) exactly as given — never held back by uncertainty about anything but the city. When you are genuinely NOT sure which CITY the user means — an ambiguous place that maps to more than one city, or a location you cannot confidently tie to a real Saudi city — ASK the one location question FIRST ("Do you mean Al Rawdah in Jeddah?", "Which city is that in?") and do NOT display property cards until the user answers. Uncertainty about anything OTHER than the city (deal, type, budget, bedrooms, amenities) is NEVER a reason to ask — use your best reading and search; the search summary shows exactly what was understood, so the user corrects it in one more message instead of being asked first. Never dump unrelated cards just to avoid asking the location question, and never claim a place doesn't exist before genuinely trying to match it.
...
- BUDGET CARRIES FORWARD: keep the user's budget across a NEW search in the same chat unless they change it (e.g. "apartment under 500k" then "now show me villas" keeps the 500k) — even if it now looks unrealistic for the new type/deal/location, NEVER ask about it (owner rule 2026-09-11: the only question is location); search with it as given. A NEW CHAT always starts fresh.
...
6. THE ONLY CLARIFYING QUESTION EZHALAH EVER ASKS IS ABOUT LOCATION, AND ONLY ONCE (owner rule 2026-09-11 — supersedes any earlier "high/medium/low confidence → 0/1/2 questions" reading; see QUESTION POLICY). For a confidently-resolved place (a clear city/district or a recognized landmark) infer the city and search without asking (e.g. near Aramco → Dhahran). A bare geography/proximity cue (near the sea, near a road) is NOT enough to infer a city — never guess one from it; leave location "" instead. NEVER guess a city, and NEVER claim a place does not exist just because it is not a landmark. If, after your one question, the city is STILL unknown, do NOT invent one and do NOT propose searching all of Saudi Arabia — Ezhalah does not run nationwide searches; the platform states that plainly and points the user to the Filter instead. If rent vs buy is still unknown, deal="Both" — never ask, never default to just Rent. "NEAR ME" / "close to my work" / "within X km" — you have NO live GPS or device location: infer the city/district from any landmark, area, or workplace they name and search immediately; if they named nothing locatable, that is exactly the one location question (ask which city/area). Never claim to know where the user physically is.
...
8. WHEN UNSURE ABOUT THE LOCATION, ASK — ONCE (owner rule 2026-09-11 — supersedes any earlier "when unsure about anything, ask" reading of this rule). If you are not sure which city/district the user means, ask (see QUESTION POLICY) — that is the ONLY question Ezhalah ever asks. For everything else — which option they meant, which budget figure, rent vs buy, a vague/unusual request — never ask: take your best, honest reading of what they said and search with it. If something is genuinely unclear or unstated, leave that filter open rather than asking or guessing a specific value; the search summary shows exactly what was understood, and the user can correct anything in one more message. This is a deliberate trade: Ezhalah would rather run an honest, slightly-broader search than spend a second question on anything but the city.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    // Deterministic question-budget hint: the user gets ONE clarifying question, ever, before the
    // first search — and it is only ever about location (owner rule 2026-09-11, 2 -> 1, and no
    // longer about ANY other field). This is a PROMPT HINT only, to save a wasted round-trip —
    // decideAgentTurn() in ./decide.ts is what actually enforces the ceiling afterward, reading
    // the structured askCount the client sent, never by regex-scanning history text (the old
    // `priorQuestions` scan this replaces).
    const budgetDirective = askCount >= 1
      ? ` IMPORTANT: you have ALREADY asked your one clarifying question in this chat — do NOT ask another, about location or anything else. IF the user's latest message is continuing a PROPERTY SEARCH (an answer to your question, or more search detail), then SEARCH NOW (kind="listings") with whatever you have: infer the city from any landmark/geography/lifestyle clue, else leave location "" (the platform will state plainly that it needs a city and point to the Filter — never propose searching all of Saudi Arabia), and deal="Both" if rent vs buy is unknown. BUT if the latest message is NOT a property search — a utility/explanation/currency-or-unit-conversion/brand/support/general-Ezhalah question, or small talk — just ANSWER it directly (kind="message"); do NOT force a search.`
      : "";
    ...
    // already decided to search on (previously a "message"-classified turn skipped both).
    {
      ...
      if (decision.kind === "message") {
        // UNSEARCHABLE — THE ONE QUESTION WAS ALREADY ASKED (owner, 2026-09-11). Checked FIRST: an
        // unresolved ambiguity or a still-missing place produces EXACTLY the same establishedState
        // facts whether this is the first ask or the ceiling has been spent — nothing below this
        // line knows about askCount. Without this branch winning first, noIntentReply/noPlaceReply
        // would silently re-render the SAME question forever once decision.unsearchable is true,
        // which is precisely the ask-loop the new ceiling exists to end. This is a STATEMENT, not a
        // question — never re-ask, for location or anything else, and never fall back to a
        // nationwide search to escape it (2026-09-04: the product does not run those).
        const unsearchableReply = decision.unsearchable
          ? (locale === "en"
              ? "Sorry, I couldn't pin down a city or district for this search. Try the Filter to search more broadly."
              : "عذراً، ما قدرت أحدد مدينة أو حي لبحثك. جرب استخدام الفلتر للبحث بشكل أوسع.")
          : null;
        ...
        const noIntentReply = !unsearchableReply && !ambiguityReply && !hasEnoughToSearch(wired.establishedState)
          ? (locale === "en"
          ...
          : null;
        const noPlaceReply = !unsearchableReply && !ambiguityReply && !noIntentReply && !hasUsableLocation(wired.establishedState)
          ? (locale === "en" ? "Which city are you searching in?" : "في أي مدينة تبحث؟")
          : null;
        // ASKING ABOUT PROPERTY TYPE IS GONE (owner, 2026-09-11 — supersedes the 2026-09-06
        // "location + type ⇒ ask" rule). Ezhalah's ONLY clarifying question is about location; a
        // resolved location with no type now falls straight through to a search covering every
        // type, never a second question. There is deliberately no noTypeReply branch any more.
        ...
        const reply = unsearchableReply ?? ambiguityReply ?? noIntentReply ?? noPlaceReply ?? oneQuestionOnly(groundReply(lead(out.reply), locale, outAmenities));
        // THIS QUESTION IS NOT OPTIONAL (owner, 2026-09-05; the client-side override this protected
        // against was DELETED 2026-09-11, not just disarmed — src/app/agent.tsx no longer keeps a
        // parallel ask-ceiling at all, it trusts this server verdict unconditionally, same as it
        // already did for kind="listings"). Historically: the client used to keep its own ask-
        // ceiling ("asked twice already and we can see some intent, so stop pestering and just
        // search"), which was right for an ordinary clarification but silently chose the user's
        // search scope for a LOCATION question — measured in production 2026-09-05, the edge
        // returned ««الرياض» اسم مدينة واسم منطقة… تقصد مدينة الرياض ولا منطقة الرياض كاملة؟» and
        // the client discarded it, searching منطقة الرياض (10,932 rows across 20 cities) instead.
        // `locationQuestion` is kept (rather than deleted along with the client override it used to
        // gate) because it is still useful signal for the client to know THIS specific message is
        // the location question — `unsearchableReply` is deliberately excluded from it, since that
        // one is a statement, not a question.
        const locationQuestion = !unsearchableReply && !!(ambiguityReply ?? noPlaceReply);
