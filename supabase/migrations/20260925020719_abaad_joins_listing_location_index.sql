-- listing_location_index gains أبعاد (abaad)'s two arms — the last piece of its onboarding.
--
...
--
-- Same machinery as 20260924171255, narrowed to abaad: the splice reads the LIVE body at apply time
-- and inserts before its closing ") u) v", every dependent is captured from the live catalog (with
-- definition, indexes and ACL read from pg_class.relacl, since information_schema has no rows for a
-- matview) and replayed in dependency order, and the "nothing was lost" guard compares the TABLES
-- THE DEFINITION READS before and after — never the row count, because this matview refreshes daily
-- (cron 16, 07:30 UTC) and a same-day rebuild legitimately drops everything deactivated since.
-- The twin is built FIRST, so a bad splice fails on CREATE and the original is never dropped.
--
-- A FIRST ATTEMPT AT 01:30 UTC HIT THE 15s lock_timeout on the DROP while a monitoring job held the
-- matview. That is the guard working: fast rollback, no orphan twin, every matview intact and
-- populated. Retried inside the :05-:15 window this file's own header prescribes.
--
-- AFTER ABAAD'S FIRST CRAWL (cannot live in a migration — the tables are empty right now):
...
begin
  if position('abaad_residential_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) > 0 then
    raise notice 'listing_location_index already carries the abaad arms - nothing to do';
    return;
  end if;
  ...
  if coalesce(array_length(before_tables, 1), 0) < 100 then  -- 141 measured 2026-09-24
  ...
  for r in select s.slug, s.slug || '_' || c.cat || '_listings' as tbl, c.cat
             from unnest(ARRAY['abaad']) with ordinality s(slug, n)
             cross join (values (1,'residential'),(2,'commercial')) c(k, cat)
            order by s.n, c.k
  loop
    if to_regclass('public.'||r.tbl) is null then
      raise exception 'table % does not exist - apply abaad_listing_tables first', r.tbl;
    end if;
    arms := arms || format($f$
UNION ALL
 SELECT ('%1$s'::text || ':'::text) || %1$I.id::text AS index_id, %1$I.id AS listing_id,
  ...
  if position('abaad_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 then
    raise exception 'listing_native_location_v1 was replayed WITHOUT abaad - apply abaad_wiring_into_search first';
  end if;
  -- The new arms are asserted by SHAPE, not by row count: abaad's tables were created minutes ago
  -- and it has not crawled yet, so both have zero rows at this moment.
  if position('abaad_commercial_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0 then
    raise exception 'the abaad splice did not take';
  end if;
  -- and the thirty-five arms spliced earlier today must have survived this CASCADE
  if position('flow_commercial_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0
     or position('dwelleo_residential_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0 then
    raise exception 'an arm spliced earlier today was lost';
  end if;
  raise notice 'listing_location_index: abaad arms spliced, % source tables kept, % dependents replayed',
