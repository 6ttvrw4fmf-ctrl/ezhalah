-- listing_location_index gains WAVE 2's eight arms — the last DB piece of their onboarding.
--
...
--
-- Same machinery as 20260925171238 (wave-1), widened to four: the splice reads the LIVE body at
-- apply time and inserts before its closing ") u) v", every dependent is captured from the live
-- catalog and replayed in dependency order, and the "nothing was lost" guard compares the TABLES THE
-- DEFINITION READS before and after — never the row count, because this matview refreshes daily
-- (cron 16, 07:30 UTC) and a same-day rebuild legitimately drops everything deactivated since. The
-- twin is built FIRST, so a bad splice fails on CREATE and the original is never dropped.
--
-- THREE PRIOR ATTEMPTS HIT THE 15s lock_timeout on the DROP — genuinely blocked, not noise: pg_locks
-- traced it to pid 14000, a fleet-wide P0 detector sweep (mon_dispatch_p0_fast) holding
-- AccessShareLock on listing_location_canonical_mv (a direct dependent) for its whole ~5-minute
-- transaction. That is the guard working exactly as designed: every attempt rolled back clean,
-- verified after each one (259,176 rows intact, zero splice). Retried once that pid's transaction
-- ended (confirmed via pg_locks: zero locks held by it moments before this run).
--
-- AFTER THE FIRST CRAWLS (cannot live in a migration — the eight tables are empty right now):
...
-- last_updated for each of the four.
--
...
begin
  if position('alajlan_residential_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) > 0 then
    raise notice 'listing_location_index already carries the wave-2 arms - nothing to do';
    return;
  end if;
  ...
  for r in select s.slug, s.slug || '_' || c.cat || '_listings' as tbl, c.cat
             from unnest(ARRAY['ibaax','remaxsa','qmra','alajlan']) with ordinality s(slug, n)
             cross join (values (1,'residential'),(2,'commercial')) c(k, cat)
            order by s.n, c.k
  loop
    if to_regclass('public.'||r.tbl) is null then
      raise exception 'table % does not exist - apply wave2_four_platforms_listing_tables first', r.tbl;
    end if;
    arms := arms || format($f$
UNION ALL
 SELECT ('%1$s'::text || ':'::text) || %1$I.id::text AS index_id, %1$I.id AS listing_id,
  ...
  if position('alajlan_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 then
    raise exception 'listing_native_location_v1 was replayed WITHOUT the wave-2 four - apply wave2_four_wiring_into_search first';
  end if;
  ...
  if position('tuba_residential_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0
     or position('abaad_commercial_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0
     or position('flow_commercial_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0 then
    raise exception 'an arm spliced earlier was lost';
  end if;
  raise notice 'listing_location_index: wave-2 arms spliced, % source tables kept, % dependents replayed',
