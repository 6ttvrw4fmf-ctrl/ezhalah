-- 2026-08-09 — three ordinary defects found while auditing the stuck search_v2_drift alert.
--
-- (1) mon_raise() could never escalate. It returned 0 the moment an open alert_event existed for
--     the dedup key, so an alert that opened at P2 stayed P2 forever, carrying the numbers it was
--     born with. dealapp_residential_listings opened `stale_active` at P2 on 2026-07-27 with
--     frac 0.234 / active 3,873 / stale_7d 905. By today the real figure is frac 0.414 /
--     active 7,212 / stale_7d 2,985 — well past the crit threshold of 0.30 — and it was still
--     displaying the three-week-old P2 payload. That is why dealapp could go 30 days without a
--     single deactivation (last kill 2026-07-10, 0 of its 274 inactive rows source-verified) and
--     never page anyone. The bug is fleet-wide: EVERY detector routes through mon_raise.
--
-- (2) location_pipeline_monitor() compared a gated column with an ungated one, so search_v2_drift
--     could never reach 0. search_listings_ar carries a BEFORE INSERT OR UPDATE trigger
--     (trg_price_size_sanity → enforce_price_size_sanity) that sets production_ready := false when
--     price_size_impossible() holds. listing_native_location_v2.production_ready is a pure location
--     expression with no price/area term. sync_search_listings_ar() has a self-repair arm for that
--     exact column, so every hour it rewrote the row, the trigger flipped it straight back, and the
--     monitor counted it as drift two minutes later — ~90 consecutive samples pinned at 33/34/36
--     since 2026-08-06 18:17 UTC. Real location drift was invisible behind that floor.
--
-- (3) listing_native_location_v2 emitted duplicate (source_table, listing_id) rows for souq24.
--     The view is a UNION ALL. Branch 4 (catch-all) excludes souq24 AND carries a NOT EXISTS guard
--     against listing_native_location_v1; the two souq24 branches carry neither. So a souq24 row
--     that is present in v1 is emitted by branch 1 AND by its souq24 branch. Exactly 2 rows today
--     (souq24_residential_listings 648152, 648160), each with a different city_id per copy, which
--     the sync's `distinct on ... order by last_updated desc nulls last` then resolves arbitrarily.
--     Both currently sit in the index with city_id NULL. This is also the documented "+2" gap
--     between crawl_stats' total_searchable and the real index count.
--
-- NOTE ON (3): the view recreation below was applied as part of this migration and then REVERTED
-- 52 seconds later by a concurrent session's migration (20260809154124_dealapp_live_location_
-- overlay), which recreated the same view from its own base text. The souq24 guard was
-- re-established by 20260809154813, which derives the new definition from pg_get_viewdef() at apply
-- time so it composes with whatever the other session landed rather than clobbering it. This file
-- is left as it was actually applied; replaying the three migrations in order yields the correct
-- final state.
--
-- Deliberately NOT changed here: the price/size plausibility gate itself. Removing its
-- production_ready arm would un-hide 28 listings whose values are confirmed source-published, but
-- it would also flip mon_detect_impossible_price_size() into a permanently unresolvable P1, and
-- weakening a safety gate is owner-approval territory (AGENTS.md). Change (2) therefore makes the
-- withheld population VISIBLE under its own name instead of silencing it — see price_gate_withheld.
...
begin
  ...
  -- never go stale or under-report: refresh the payload on every run so the dashboard shows
  -- today's numbers, and if the condition has got WORSE, promote the severity and re-arm both
  -- dispatch and acknowledgement so the escalation actually reaches a human. mon_dispatch_alerts()
  -- only ever sends rows with dispatched_at IS NULL, so clearing it is what makes the page happen.
...
comment on function public.mon_raise(text,text,text,text,jsonb) is
  'Raise or update one open alert per dedup_key. Refreshes detail every call; escalates severity '
  'and re-arms dispatch when the condition worsens. Returns 1 on a new alert or an escalation.';
...
begin
  ...
  -- otherwise this migration would silently bury a population that includes listings whose prices
  -- the source platform genuinely publishes. Owner decision pending; the number must stay in view.
  ...
  if v_withheld > 0 then
    perform public.mon_raise('P2','price_gate_withheld','search_index','price_gate_withheld',
      jsonb_build_object('count', v_withheld,
        'why','listings withheld from search by the price/size plausibility gate (trg_price_size_sanity). '
              'Verified 2026-08-09: 28 of 34 carry values the source platform itself publishes, so the '
              'gate is hiding source-published data — contrary to the standing rule that a source price '
              'stays searchable at any magnitude. Needs an owner decision; the remainder are parser bugs.'));
  else
  ...
  -- dealapp district-resolution gap — production_ready dealapp rows with NO district that DO have a
...
-- ---------------------------------------------------------------------------------------------
-- The guard is deliberately tied to "branch 1 will ACTUALLY emit a replacement" (v1 AND
-- active_listing_ids_v2 membership), not to v1 membership alone. Both are matviews read against a
-- live source table, so during refresh skew a souq24 row can be active + in v1 + absent from
-- active_listing_ids_v2. A v1-only guard would suppress the souq24 branch while branch 1 stayed
-- silent, the row would vanish from v2, and the sync's DELETE arm would then drop it from the
-- search index — hiding a source-published listing. This form is a strict de-duplication: it can
-- only ever remove a second copy, never the last one.
