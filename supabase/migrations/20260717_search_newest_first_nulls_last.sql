-- ─────────────────────────────────────────────────────────────────────────────────────────
-- "Newest first" ordering fix (owner 2026-07-16): location_search_candidates_ar ordered by
-- `last_updated DESC NULLS FIRST`, which floated every unknown-date (last_updated IS NULL) listing
-- to the very TOP of every search. Live-verified those NULL rows are a MIX of new and old listings
-- (their raw last_seen_at only means "still live", NOT "newly posted"), so surfacing them first
-- fabricated a recency they don't actually have — the opposite of "newest first".
--
-- Fix: `NULLS FIRST` → `NULLS LAST` in all four ORDER BY / row_number() clauses. Sort strictly by
-- the REAL source date (last_updated); a listing with no known date goes LAST, never first. Nothing
-- is invented, no newer timestamp is fabricated, and the filter WHERE clause + the two-scope +
-- per-platform-seed structure are byte-identical — so Match → Diversify → Return and filter accuracy
-- are fully preserved (the client-side mergeDiversitySeed recency tiebreak was changed to match, in
-- src/lib/platformDiversity.ts, same commit).
--
-- Applied LIVE via Supabase MCP 2026-07-16; this file mirrors exactly what was applied. Only the four
-- `nulls first` → `nulls last` tokens changed vs the live definition; every filter line is unchanged.
