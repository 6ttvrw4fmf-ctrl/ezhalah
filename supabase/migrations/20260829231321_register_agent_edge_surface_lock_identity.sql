...
-- gets ignored.
--
-- NEEDLE EDIT off the LIVE definition (memory: RPC full-body-replace revert hazard): anchor on the
-- existing known identity and add beside it. Idempotent; refuses to guess if the anchor is gone.
--
-- MIRROR RECOVERED VERBATIM from supabase_migrations.schema_migrations (version 20260829231321) on
-- 2026-08-30. It was applied to production without a matching git file, which failed the schema-drift
-- gate closed and blocked every production deploy — the documented remedy is exactly this recovery.
do $do$
declare def text;
begin
