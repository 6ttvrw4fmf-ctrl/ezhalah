-- REPLAY CHECKPOINT (2026-08-15): literal definitions of the three shared AF RPC surfaces,
-- byte-identical to production at checkpoint time (asserted below against af_rpc_build_state).
-- Exists so scripts/lib/rpcReplay.ts resolves a single effective body: migrations 20260815012506
-- and 20260815014102 rewrote these functions dynamically (pg_get_functiondef-based), which the
-- replayer correctly refuses to model; a full literal definition wipes that slate. The bodies
-- below were REGENERATED via rebuild_af_filter_rpcs() from af_rpc_templates + af_eligibility_clause
-- by migration af_clause_adopt_both_period_and_rebaseline immediately before this checkpoint —
...
-- Byte-identity guard: this checkpoint must be a NO-OP. If any body above ever disagrees with
-- af_rpc_build_state (the rebuild ledger), applying this file would be a hand-edit — abort.
do $chk$
