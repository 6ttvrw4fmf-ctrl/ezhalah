-- SNAPSHOT of the two Monthly counts RPCs, taken 2026-08-09 from pg_get_functiondef.
-- WHY: their migration history ends in needle-edit patchers whose shape a static replayer cannot
-- safely interpret, which left the repo with no readable current definition — the exact condition in
-- which a guard can be deleted while CI stays green. Bodies are byte-identical to production, so
-- re-running is a no-op; the point is that replay now ends here via a plain CREATE OR REPLACE.
-- NOTE: location_search_candidates_ar is intentionally NOT snapshotted here (see the open bedrooms
-- question) — it already replays cleanly.
