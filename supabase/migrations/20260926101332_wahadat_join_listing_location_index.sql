-- listing_location_index gains wahadat's two arms — the last DB piece of its onboarding.
--
-- listing_location_index is the ONLY source of last_updated. A platform without an arm here gets
-- NULL, sorts last, and vanishes when the user scrolls — the exact bug wave-1 and wave-2 both hit.
--
-- Machinery is 20260926043530 (the wave-2 four) narrowed to two tables: capture every dependent,
-- refuse to proceed on a broken probe, splice the arms into the body read LIVE at apply time, build
-- a populated __mig twin, swap, replay dependents. Any failed assertion rolls the whole file back.
--
-- AFTER THIS LANDS the refresh chain must run IN THIS ORDER before the next sync, or wahadat's rows
...
begin
  if position('wahadat_residential_listings' in
              pg_get_viewdef('public.listing_location_index'::regclass,true)) > 0 then
    raise notice 'listing_location_index already carries the wahadat arms - nothing to do';
    return;
  end if;
  ...
  raise notice 'captured % dependent(s): %', (select count(*) from _dep_capture),
  ...
  for r in select s.slug, s.slug || '_' || c.cat || '_listings' as tbl, c.cat
             from unnest(ARRAY['wahadat']) with ordinality s(slug, n)
             cross join (values (1,'residential'),(2,'commercial')) c(k, cat)
            order by s.n, c.k
  loop
    if to_regclass('public.'||r.tbl) is null then
      raise exception 'table % does not exist - apply wahadat_listing_tables first', r.tbl;
    end if;
    arms := arms || format($f$
UNION ALL
...
begin
  if position('wahadat_residential_listings' in pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0
     or position('wahadat_commercial_listings' in pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0 then
    raise exception 'wahadat did not land in listing_location_index';
  end if;
  if position('ibaax_commercial_listings' in pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0
     or position('tuba_residential_listings' in pg_get_viewdef('public.listing_location_index'::regclass,true)) = 0 then
    raise exception 'an existing platform arm was lost while adding wahadat';
  end if;
  ...
  raise notice 'wahadat is in listing_location_index; refresh index -> canonical_mv -> v1 -> sync before trusting last_updated';
end
$verify$;
