...
--
-- `P and not R and (Q or (P and R))` with Q = "the same column also equals سنوي" is a
-- contradiction: no row state can satisfy it. Proven 2026-08-15 with 31,859 genuine-monthly
-- rows live plus one hand-built maximally-adversarial row — the check returned 0 rows.
--
...
-- from 21,019 to 51,379 (+30,360 genuine-monthly listings priced per MONTH shown as ANNUAL)
...
-- returning 0. Every other check derives from the same clause, so they drift together; this is
-- the only INDEPENDENT oracle for the period bucket and it was inert.
--
...
-- Needle-edit of the live definition (never a hand-typed full-body replace).
--
-- Mutation-proven: clean = 0 rows; same clause drift = n 30,360 and one hourly alert via
...
begin
  ...
  newdef := replace(def,
$old$  select 'genuine_monthly_leaked_into_annual'::text,
  ...
  having count(*) > 0;$old$,
$new$  select 'genuine_monthly_leaked_into_annual'::text,
  ...
  having count(*) > 0;$new$);
  if newdef = def then
    raise exception 'mon_filter_parity_barrier check (2) needle not found — inspect the live definition before re-applying';
  end if;
  execute newdef;
end $mig$;
