-- Wave 1: ten platforms onboarded in one pass, 2026-09-25 — their listing tables. Nothing here is
-- visible to users on its own; the companion migrations wire the tables into search.
--
--   alsaedan (SDN)  ego (EGO)  muhaysini (MHS)  nofodh (NFD)  razre (RAZ)
--   reinvest (RNV)  safa (SAF)  sokok (SKK)  sukna (SKN)  tuba (TBA)
--
-- Same shape as the thirty-five (20260924170534), the eleven (20260921185629) and abaad
...
--
-- Every one of the ten writes through db._wasalt_batch onto this same column set. Their row literals
-- were AST-read before this migration was written — scrapers/common/tests/
-- test_scraper_rows_only_use_real_columns.py, NOT_YET_ONBOARDED, which runs in CI and passed 67
-- checks over all ten — so every key is inside the template + city_ar/district_ar/city_id/region_id.
-- That read is why the four location columns are added HERE and not later: one unknown key makes
-- PostgREST reject the ENTIRE batch with PGRST204, and 20260919065332 lost 1,809 mapped rows that way.
--
-- BOTH tables are created for every platform even where a scraper writes only one of them (razre,
-- sokok and sukna are sale-side developers today), because the fleet's allowlists, RPCs and verifiers
-- enumerate two per platform — verify-all-platforms-have-extra-attrs-branch is TABLE-grained, so a
-- missing twin reads as a coverage gap rather than as an absent product line.
--
-- Two things LIKE ... INCLUDING ALL does NOT carry over, both closed here exactly as the previous
-- three passes closed them:
...
--     source_capture. A column REVOKE cannot carve a hole in a table-level grant, so the table grant
--     is revoked and the column list granted back.
--
-- CREATE TABLE takes no lock on anything that already exists and the ten names are new (verified: 0
-- of the 20 exist), so this is safe outside the :05-:15 DDL window. The view and matview arms, which
-- do take ACCESS EXCLUSIVE on objects the :22 cron reads, are separate migrations.
DO $$
DECLARE p text; t text; tbl text; cols text;
BEGIN
  FOREACH p IN ARRAY ARRAY['alsaedan','ego','muhaysini','nofodh','razre',
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'all 20 tables present: location columns, UNIQUE (ad_number), fleet triggers, PDPL column grant';
END $verify$;
