-- listing_location_index gains arms for the thirty-five platforms onboarded today.
--
...
--
-- based on LIVE definition, md5(pg_get_viewdef('public.listing_location_index'::regclass, true)) = 018e7c74ca96b1898511a9f9169cf5f5 (185,241 chars) fetched 2026-09-24 15:02:27 UTC
-- The splice reads the LIVE body at apply time and inserts before its closing ") u) v" — i.e. right
-- after the nufouth commercial arm, the last one in the live text (verified 2026-09-24) — so a change
-- another session made in between is carried forward, never reverted by a pasted body.
--
-- RECIPE (20260920003543, with the two corrections 20260921191139 kept):
...
--     Measured 2026-09-24 the CASCADE reaches 13 objects (unchanged since #3516), two of them
--     MATVIEWS with indexes (listing_location_canonical_mv, listing_native_location_v1). No function,
--     constraint or trigger depends on any of them.
...
-- thirty-five platforms' rows carry NULL last_updated and sort last. After the first complete
...
-- non-NULL last_updated for the thirty-five platforms.
--
...
begin
  if position('dwelleo_residential_listings' in
  ...
  for r in select s.slug, s.slug || '_' || c.cat || '_listings' as tbl, c.cat
             from unnest(ARRAY['dwelleo','aqalemhajer','sakani','shatri','alqasem','fkralemar',
                          ...
                          'rightcompound','livingcompound','azure','expattrusted','flow']) with ordinality s(slug, n)
             cross join (values (1,'residential'),(2,'commercial')) c(k, cat)
            order by s.n, c.k
  loop
    if to_regclass('public.'||r.tbl) is null then
      raise exception 'table % does not exist - apply the thirty_five_platforms_tables migration first', r.tbl;
    end if;
    arms := arms || format($f$
UNION ALL
 SELECT ('%1$s'::text || ':'::text) || %1$I.id::text AS index_id, %1$I.id AS listing_id,
  ...
  if position('dwelleo_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 then
    raise exception 'listing_native_location_v1 was replayed WITHOUT the thirty-five arms - apply the wiring migration first';
  end if;
  ...
  raise notice 'listing_location_index: thirty-five platform arms spliced, % source tables kept, % dependents replayed',
