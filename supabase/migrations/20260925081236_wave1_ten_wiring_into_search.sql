-- Wave 1's ten platforms join search: their twenty tables become arms of
-- listing_native_location_v1 and active_listing_ids_v2. Until this runs, the twenty tables created
-- by 20260925080640 are tables no search can reach.
--
--   alsaedan  ego  muhaysini  nofodh  razre  reinvest  safa  sokok  sukna  tuba
--
-- THIS IS THE 20260925012657 MACHINERY VERBATIM (itself 20260924170648's), with only the platform
-- list, the presence guards and the post-swap roster changed. Those two migrations wired abaad and
-- thirty-five platforms through these exact steps, so this is the proven path rather than fresh DDL
-- surgery on the matview the whole product reads: capture every dependent's definition/indexes/grants
-- first, refuse to touch anything if the probe returns zero dependents or an empty definition, build
-- a `__mig` twin, assert the twin still reads every source table the live body read, swap, then
-- replay the dependents. Any failed assertion raises and the whole transaction rolls back.
--
...
--
-- The post-swap loop checks all ten landed AND that abaad and the thirty-five wired on 09-24 are
-- still there, because this CASCADEs the same matviews they depend on.
...
DECLARE
  ...
  wire_v1 boolean := position('tuba_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0;
  wire_v2 boolean := position('tuba_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0;
BEGIN
  IF NOT wire_v1 AND NOT wire_v2 THEN
    RAISE NOTICE 'both matviews already carry the wave-1 ten - nothing to do';
    RETURN;
  END IF;
  FOREACH t IN ARRAY tables LOOP
    IF to_regclass('public.' || t) IS NULL THEN
      RAISE EXCEPTION 'table % does not exist - apply wave1_ten_platforms_listing_tables first', t;
    END IF;
  END LOOP;
...
BEGIN
  FOREACH t IN ARRAY ARRAY['alsaedan','ego','muhaysini','nofodh','razre',
                           'reinvest','safa','sokok','sukna','tuba',
                           'abaad',
  ...
  END IF;
  RAISE NOTICE 'the wave-1 ten are wired into search; abaad, the thirty-five and every dependent intact';
END
$verify$;
