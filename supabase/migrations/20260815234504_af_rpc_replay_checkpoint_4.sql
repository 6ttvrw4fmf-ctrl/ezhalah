-- REPLAY CHECKPOINT 4 (2026-08-16): literal definitions of the three shared AF RPC surfaces
-- immediately after af_amenity_tokens_electricity_water_supply (commercial cohort backend),
-- byte-identical to production (asserted below). Same slate-wipe semantics as checkpoints 1-3;
-- the template path remains the only sanctioned editor; this file changes nothing.
...
AS $function$
  with district_tokens as (
    ...
    select s.rent_now_pay_later, s.kitchen, s.parking, s.elevator, s.furnished, s.bathrooms, s.air_conditioner, s.private_entrance, s.maid_room, s.driver_room, public.norm_direction_ar(s.direction_ar) as dirn, s.street_width_m, s.car_entrance, s.sanitation, s.electricity, s.water_supply
    from public.search_listings_ar s
  ...
  select
    ...
    count(*) filter (where water_supply)                    as cnt_water_supply,
