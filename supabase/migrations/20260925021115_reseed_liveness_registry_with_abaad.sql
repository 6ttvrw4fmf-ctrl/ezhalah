-- ops_liveness_registry reseeded so it lists أبعاد (abaad) alongside every platform already in it.
--
-- WHY A FULL RESEED AND NOT A ONE-LINE INSERT: verify-liveness-registry-mirror holds THREE things to
-- the same content — scrapers/common/liveness_policies.py, sql/mirrors/liveness_registry.json, and
-- the newest reseed migration's own seed — and it also requires the delete clause below, because an
...
-- rather than by hand-editing three files and hoping.
