-- Re-seed the WHOLE liveness registry from sql/mirrors/liveness_registry.json (40 platforms),
-- now including remal and amaall.
--
-- WHY THE WHOLE THING: verify-liveness-registry-mirror.ts holds the LATEST seed migration to EXACT
-- equality with the JSON mirror, and separately requires a retention delete — an insert-only
-- two-row seed fails both checks, and the delete matters in production too: without it a platform
-- that leaves the registry keeps being reported as covered.
--
-- remal and amaall are registered at CRAWL_PRESENCE_ONLY/168h/grace 3 — the honest tier for a small
-- WordPress catalogue re-read in full each run with no per-listing revisit endpoint. amaall
-- additionally publishes an explicit property_status (تم البيع / تم التأجير / تم التأجير بالكامل)
-- which its scraper reads directly; that is SOURCE-STATED removal and deactivates through the
-- scraper, never through this monitoring tier.
