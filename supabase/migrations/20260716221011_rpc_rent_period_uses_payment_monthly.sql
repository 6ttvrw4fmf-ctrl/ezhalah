-- Recovered verbatim from production on 2026-07-20 (drift reconciliation): applied directly to prod via MCP as supabase_migrations.schema_migrations version 20260716221011, name 'rpc_rent_period_uses_payment_monthly'; this committed file is the source-of-truth record, NOT the apply path. Body is byte-for-byte the live statement (md5 982eb1f990edda01d4d96132351d8701).

-- Owner PERMANENT rule (2026-07-16): the Monthly/Yearly filter now buckets on PAYMENT SCHEDULE
-- (payment_monthly), not lease length. Only the rent-period predicate changes; every other clause
-- (category purity, location, price ×12, beds/area/age/etc, the not-materialized `matched` CTE and the
-- per-platform union-all) is preserved byte-for-byte from the current deployed definition.
--   Monthly ('شهري') = payment_monthly true  (source explicitly offers monthly payment)
--   Yearly  ('سنوي') = payment_monthly false (must pay annually)
