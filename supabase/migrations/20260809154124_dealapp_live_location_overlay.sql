-- Dealapp live location overlay (2026-08-09). Catchall branch gets a LIVE listings_arabic_locations
-- overlay (dealapp-scoped, CONFIDENT single catalogued-city match only), so a resolved dealapp row
-- is searchable without waiting for the hourly v1 matview refresh. Never guesses. Clobber-safe.
CREATE OR REPLACE VIEW public.listing_native_location_v2 AS
 SELECT v1.platform,
   ...
   FROM listing_native_location_v1 v1
     ...
     LEFT JOIN loc_catalog_region ur ON ur.region_id = uc.city_id
UNION ALL
