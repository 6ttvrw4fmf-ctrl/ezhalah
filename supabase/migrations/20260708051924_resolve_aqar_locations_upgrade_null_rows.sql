-- Second fix (owner 2026-07-08): the shadow-insert blocks used ON CONFLICT DO NOTHING, so listings that
-- already had a null-city row could never be upgraded once the resolver learned to match them. Changed to
-- DO UPDATE guarded by "parsed_city_id IS NULL" so a row is only ever filled in, never overwritten once
-- resolved. Combined with the normalize+alias join, the resolver is now self-healing for null-city rows.
...
begin
  ...
  with ins as (
    ...
    join loc_catalog_region cr on cr.region_ar = cm.region_ar
    join loc_catalog_city cc on cc.region_id = cr.region_id and (normalize_ar(cc.city_ar) = normalize_ar(cm.city_ar) or exists (select 1 from loc_catalog_city_alias al where al.alias_norm = normalize_ar(cm.city_ar) and al.city_id = cc.city_id))
    join loc_catalog_district d on d.city_id = cc.city_id and d.district_norm = normalize_ar(a.neighborhood)
