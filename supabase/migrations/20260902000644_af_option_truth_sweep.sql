-- ops_af_option_truth_sweep — THE Advanced-Filter button, proven option by option.
--
-- THE CONTRACT (owner, 2026-09-01): if a chip says N, then selecting that chip must return exactly
-- those N listings, and every one of them must really carry the attribute per the certified column.
-- Three things have to agree, and this proves all three in one pass, per option, per cohort:
--
--   chip     = apartment_guided_counts_ar's cnt_* — THE NUMBER PRINTED ON THE BUTTON
--   applied  = af_eligible_count with that option applied — THE FILTER THE SEARCH RUNS
--   returned = distinct listing_ids location_search_candidates_ar actually hands back
--   viol     = of those rows, how many FAIL the predicate when read straight off the column
--
-- Any row this function returns is a defect. An empty result is the proof.
--
-- WHY A FOURTH COLUMN AND NOT JUST chip=applied. A count and a result set can agree with each other
-- and both be wrong. `viol` reads the listing's own column, so it catches a predicate that filters on
-- the wrong field entirely — the one failure mode the first three share a blind spot for.
--
-- WHY IT LIVES IN THE DB. The chip count and the search predicate are generated from ONE shared
-- clause (af_eligibility_clause) into three different RPCs. Only the database can compare what those
-- three actually do; a repo-side test can compare their SOURCE and be green while the deployed
-- functions disagree — which is exactly how the direction-chip defect survived (chip 0, tap +320).
...
begin
  for c in
    ...
    order by r.deal_ar, r.rent_period_ar nulls first, r.type_ar
  loop
...
$fn$;
comment on function ops_af_option_truth_sweep(text,text,text,int,boolean) is
  'AF button truth: per cohort per option, chip count = applied filter = returned listings, and every returned row satisfies the predicate on its own column. Any returned row is a defect.';
