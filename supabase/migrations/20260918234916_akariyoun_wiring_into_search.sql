-- عقاريون JOINS SEARCH — launch-checklist boxes 3 and 4.
-- Same capture-and-replay shape as 20260914081716 (rakez): the dependent views are read out of the
...
BEGIN
  ...
  IF position('akariyoun_residential_listings.city_ar' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) > 0 THEN
  ...
  arms := '';
  IF position('akariyoun_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) > 0 THEN
...
BEGIN
  IF position('akariyoun_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'akariyoun did not land in listing_native_location_v1';
  END IF;
  IF position('akariyoun_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'akariyoun did not land in active_listing_ids_v2';
  END IF;
  ...
  IF position('rakez_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'rakez was lost while wiring akariyoun';
  END IF;
  IF position('suwar_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'suwar was lost while wiring akariyoun';
  END IF;
