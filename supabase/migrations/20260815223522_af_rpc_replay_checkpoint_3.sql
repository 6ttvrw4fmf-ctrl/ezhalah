-- REPLAY CHECKPOINT 3 (2026-08-16): literal definitions of the three shared AF RPC surfaces
-- immediately after af_amenity_tokens_car_entrance_sanitation (villa cohort backend), byte-identical
-- to production (asserted below against af_rpc_build_state). Same slate-wipe semantics as the two
-- prior checkpoints: the token migration rebuilds dynamically via rebuild_af_filter_rpcs(), which
-- scripts/lib/rpcReplay.ts refuses to model; this literal snapshot is what a fresh replay resolves
-- to. The template path remains the only sanctioned editor; this file changes nothing.
    ...
    where (s.production_ready or ((p_cities is null or cardinality(p_cities) = 0) and (p_districts is null or cardinality(p_districts) = 0) and p_region_ids is null and not public.search_row_price_gated(s.deal_ar, s.price_total) and (s.region_id is null or s.city_id is null)))
      ...
      and (p_amenities is null or (
               not exists (select 1 from unnest(p_amenities) tok
                           where tok not in ('elevator','parking','kitchen','ac','maid_room','driver_room','private_entrance','car_entrance','sanitation','furnished','rnpl','rent_now_pay_later'))
...
AS $function$
  with district_tokens as (
    ...
    select s.rent_now_pay_later, s.kitchen, s.parking, s.elevator, s.furnished, s.bathrooms, s.air_conditioner, s.private_entrance, s.maid_room, s.driver_room, public.norm_direction_ar(s.direction_ar) as dirn, s.street_width_m, s.car_entrance, s.sanitation
    from public.search_listings_ar s
  ...
  select
    ...
    count(*) filter (where sanitation)                      as cnt_sanitation,
