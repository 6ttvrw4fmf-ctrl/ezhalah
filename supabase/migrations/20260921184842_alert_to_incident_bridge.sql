...
select
  ...
  (select round(extract(epoch from (now()-min(created_at)))/86400.0,1) from alert_event where resolved_at is null and severity in ('P0','P1')) as oldest_serious_days,
  (select count(*) from alert_event where resolved_at is null and last_affirmed_at is not null and last_affirmed_at < now()-interval '48 hours') as stale_unworked,
  (select count(*) from alert_event where resolved_at is null and owner_routine is null) as unrouted,
  (select count(*) from alert_event where resolved_at is null and dispatched_at is not null and acknowledged_at is null and dispatched_at < now()-interval '48 hours') as dispatched_unacked_48h,
  (select round(100.0 * count(*) filter (where verified) / nullif(count(*),0), 1) from ops_remediation_attempt where attempted_at > now()-interval '7 days') as autofix_success_pct_7d,
  ...
  (select count(*) filter (where a.last_ran is null or a.last_ran < now()-interval '30 hours')
     from (select o.canonical, (select max(h.ran_at) from ops_routine_sentry_heartbeat h join ops_routine_heartbeat_alias al on al.emits_as=h.routine where al.canonical=o.canonical) as last_ran
