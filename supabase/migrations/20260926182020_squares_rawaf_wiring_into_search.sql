-- Squares and Rawaf join search: their four tables become arms of listing_native_location_v1 and
-- active_listing_ids_v2. Until this runs, the tables created by squares_rawaf_listing_tables are
-- tables no search can reach.
--
--   squares (شركة المربعات العقارية) — 16 listable of 18 posts, measured 2026-09-26
--   rawaf   (رواف)                    — 19 AVAILABLE units of 215, measured 2026-09-26
--
-- THIS IS THE 20260926094922 (wahadat) / 20260926042615 (wave-2) MACHINERY, with only the platform
-- list, the presence guards and the post-swap roster changed: capture every dependent's
-- definition/indexes/grants first, refuse to touch anything if the probe returns zero dependents or
-- an empty definition, build a `__mig` twin, assert the twin still reads every source table the
-- live body read, swap, then replay the dependents. Any failed assertion rolls the whole file back.
--
-- The twin is CREATE MATERIALIZED VIEW ... AS, which POPULATES, so no window exists in which search
-- reads an empty index. Both platforms' tables are still EMPTY at this point, so this changes no
-- user-visible row either way.
--
...
--
-- NOTE ON RUNTIME: this transaction populates two large matviews and regularly outruns the admin
-- connector, which then reports a timeout while the transaction is still running and goes on to
-- COMMIT. Do not retry on a timeout — check pg_stat_activity for the backend first
-- (feedback: a connector timeout is not a rollback).
...
DECLARE
  ...
  tables text[] := ARRAY[
    'squares_residential_listings','squares_commercial_listings',
    'rawaf_residential_listings','rawaf_commercial_listings'];
  wire_v1 boolean := position('squares_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0;
  wire_v2 boolean := position('squares_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0;
BEGIN
  IF NOT wire_v1 AND NOT wire_v2 THEN
    RAISE NOTICE 'both matviews already carry squares + rawaf - nothing to do';
    RETURN;
  END IF;
  FOREACH t IN ARRAY tables LOOP
    IF to_regclass('public.' || t) IS NULL THEN
      RAISE EXCEPTION 'table % does not exist - apply squares_rawaf_listing_tables first', t;
    END IF;
  END LOOP;
...
BEGIN
  FOREACH t IN ARRAY ARRAY['squares','rawaf','wahadat','ibaax','qmra','tuba','abaad'] LOOP
  ...
  END IF;
  RAISE NOTICE 'squares + rawaf are wired into search; wahadat, the wave-2 four and every dependent intact';
END
$verify$;
