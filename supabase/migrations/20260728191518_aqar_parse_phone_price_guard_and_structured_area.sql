-- P0 fidelity fix (2026-07-28): stop broker phones / REGA-ID artifacts being stored as Buy prices,
-- and make the structured «المساحة» block authoritative for area. Built from the LIVE trg_aqar_parse
-- definition; ONLY the two guarded blocks below are added. price_per_meter still untouched
-- (verify-aqar-trigger-preserves-source-ppm.ts must stay green).
--
-- Root cause: scrapers/aqar/enrich_residential.py fallback pattern `(\d{6,9})\s*[§ر﷼]` treats a BARE
-- «ر» (a super-common Arabic letter) as currency, so it grabs any 6–9 digit run (phone/licence/ID)
-- next to a «ر»-word. aqar_parse() only accepts §/ريال/﷼ prices and OVERRIDES the Python value when it
-- finds one — but when it finds none (price stated in words like «380 الف», per-unit, or «على السوم»)
-- the Python garbage survived. A blanket "make aqar_parse authoritative" was REJECTED: measured live it
-- would NULL ~33% of Buy prices, because a bare «ر» is ALSO how many legit aqar prices are marked.
