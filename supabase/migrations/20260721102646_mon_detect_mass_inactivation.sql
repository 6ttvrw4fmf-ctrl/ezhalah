...
-- deploy drift gate flagged it as missing_in_git). Idempotent (on conflict do nothing / create or
-- replace; mon_run_all_detectors is a full-body replace at the same zero-arg signature, no overload).
