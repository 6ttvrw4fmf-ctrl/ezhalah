...
-- in place). Removed two columns that were false positives:
--   • buy_token_price_servable — sub-1000 Buy prices are SOURCE-REAL (PR#362, 204/204 live-verified;
--     sanadak «للبيع 868», hajer «550ر.س» — advertiser data-entry conventions). Asserting it=0 was the
--     wrong premise; price magnitude is watched by PR#362's mon_detect_price_magnitude_gate, not here.
--   • period_payment_mismatch — flagged the owner's DELIBERATE rent-now-pay-later exclusion
--     (migration 20260719113707) as a bug. False positive (aqar 23235 is RNPL, correctly non-monthly).
-- Kept: cityid_not_in_match (Taif↔Makkah compound-label class, fixed by the reconcile trigger) and
-- untaxonomized_type (a type_ar unreachable by any type filter).
