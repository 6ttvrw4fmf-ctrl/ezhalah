...
begin
  ...
  loop
    ...
    -- Create under a fixed valid surface, then set the owner DIRECTLY (below). Drift-proof: no
    -- per-routine slug map to go stale; incident_known_owners() stays the ONE roster.
    v_id := public.incident_open('alert:' || a.dedup_key,
