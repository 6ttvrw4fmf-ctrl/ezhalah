-- Wahadat joins search: its two tables become arms of listing_native_location_v1 and
-- active_listing_ids_v2. Until this runs, the two tables created by 20260926094015 are tables no
-- search can reach.
--
--   wahadat (وحدات) — 968 available units measured across all 100 project pages, 2026-09-26
--
...
--
-- The post-swap loop checks wahadat landed AND that the wave-2 four, wave-1's tuba and abaad are
...
DECLARE
  ...
  tables text[] := ARRAY[
    'wahadat_residential_listings','wahadat_commercial_listings'];
  wire_v1 boolean := position('wahadat_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0;
  wire_v2 boolean := position('wahadat_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0;
BEGIN
  IF NOT wire_v1 AND NOT wire_v2 THEN
    RAISE NOTICE 'both matviews already carry wahadat - nothing to do';
    RETURN;
  END IF;
  FOREACH t IN ARRAY tables LOOP
    IF to_regclass('public.' || t) IS NULL THEN
      RAISE EXCEPTION 'table % does not exist - apply wahadat_listing_tables first', t;
    END IF;
  END LOOP;
...
BEGIN
  FOREACH t IN ARRAY ARRAY['wahadat','ibaax','remaxsa','qmra','alajlan','tuba','abaad'] LOOP
  ...
  END IF;
  RAISE NOTICE 'wahadat is wired into search; the wave-2 four, tuba, abaad and every dependent intact';
END
$verify$;
