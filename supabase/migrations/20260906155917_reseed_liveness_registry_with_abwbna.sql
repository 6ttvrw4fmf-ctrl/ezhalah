-- Re-seed the WHOLE liveness registry from sql/mirrors/liveness_registry.json (41 platforms),
-- now including abwbna. Full re-seed + retention delete, per the pattern
-- verify-liveness-registry-mirror.ts requires (an insert-only row fails both the exact-equality and
-- the retention-delete checks).
--
-- abwbna is registered CRAWL_PRESENCE_ONLY/168h/grace 3 — the same tier aldarim already carries,
-- because abwbna is a different tenant of the identical Nuzul SaaS platform: a full-refresh crawl
-- of the complete current inventory each run, with anything not seen pruned.
