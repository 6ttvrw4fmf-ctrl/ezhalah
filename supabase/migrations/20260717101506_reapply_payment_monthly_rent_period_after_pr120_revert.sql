-- Recovered verbatim from production on 2026-07-20 (drift reconciliation): applied directly to prod via MCP as supabase_migrations.schema_migrations version 20260717101506, name 'reapply_payment_monthly_rent_period_after_pr120_revert'; this committed file is the source-of-truth record, NOT the apply path. Body is byte-for-byte the live statement (md5 15fc1498baa5695275a0417ebd252587).

-- REGRESSION FIX (2026-07-17). Migration 20260716221011_rpc_rent_period_uses_payment_monthly moved the
-- Monthly/Yearly rent filter onto s.payment_monthly (PAYMENT SCHEDULE, owner's permanent rule). 139
-- seconds later, 20260716221230_search_newest_first_nulls_last (PR#120) re-issued CREATE OR REPLACE
-- FUNCTION location_search_candidates_ar from a body copied from the PREVIOUS RPC migration
-- (20260716114027_batch4_rpc_drop_dead_sort) — carrying the STALE rent-period clause and silently
-- reverting the fix. Verified blast radius = exactly that one clause: PR#120's own NULLS LAST ordering
-- and the category-purity clause (20260716091736) both remained live and are PRESERVED verbatim here.
--
...
--   returned by the YEARLY (سنوي) filter; 15,419 payment_monthly=true rows were MISSING from MONTHLY
--   (15,221 labelled سنوي + 198 with rent_period_ar NULL). Repro: Riyadh + شقة + Rent + سنوي ->
--   total_count 9,795, of which 2,561 had payment_monthly=true.
--
-- This is a FORWARD migration (history is never edited). Body = the CURRENT live definition with ONLY the
-- rent-period predicate restored to:
--   Monthly ('شهري') = s.payment_monthly = true   (source explicitly offers monthly payment)
--   Yearly  ('سنوي') = s.payment_monthly = false  (tenant must pay annually)
