-- أملاك الأحساء JOINS SEARCH — owner-approved 2026-09-12, first of the 4 new-office platforms.
-- Wires amlakalahsa into BOTH matviews every search read actually depends on:
--   listing_native_location_v1 (native district/city/region resolution — same arm shape as every
--   other native-scraper platform) and active_listing_ids_v2 (the price/area/rooms feed every
--   platform's rows come from). Mirrors the proven 20260911214511 (remal) pattern exactly,
--   including the CASCADE-then-restore discipline for listing_native_location_v2 + its own 3
--   dependents — verified fresh against the LIVE definitions moments before writing this file, not
--   a remembered copy (per this session's own hard-won rule).

-- ── 1. Wire into listing_native_location_v1 (native arm) ───────────────────────────────────────
DO $do$
DECLARE
  base text; arms text := ''; t text; anchor text;
BEGIN
  IF position('amlakalahsa_residential_listings.city_ar' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) > 0 THEN
...
-- ── 2. Wire into active_listing_ids_v2 (the price/area/rooms feed) ─────────────────────────────
DO $do$
DECLARE
  base text; arms text := ''; t text; anchor text;
BEGIN
  IF position('amlakalahsa_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) > 0 THEN
...
-- ── 3. Restore listing_native_location_v2 — CASCADEd by EITHER drop above. Body pulled fresh from
--    the LIVE view moments before writing this migration (byte-verified against what this session
--    already had — unchanged since 20260911214511's own re-verification of the gathern overlay fix). ──
...
-- ── 4. Restore v2's own 3 dependents — a DROP CASCADE on either matview above transitively drops
--    these too. Unchanged bodies (verified fresh moments before this migration). ──────────────────
...
-- ── 5. Grants — a MATERIALIZED VIEW does not auto-inherit schema default privileges. ─────────────
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE
  ON public.listing_native_location_v1, public.active_listing_ids_v2
...
BEGIN
  IF position('amlakalahsa_residential_listings.city_ar' in pg_get_viewdef('public.listing_native_location_v1'::regclass, true)) = 0 THEN
    RAISE EXCEPTION 'amlakalahsa not wired into listing_native_location_v1';
  END IF;
  IF position('amlakalahsa_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass, true)) = 0 THEN
    RAISE EXCEPTION 'amlakalahsa not wired into active_listing_ids_v2';
  END IF;
  SELECT count(*) INTO v_n FROM listing_native_location_v2 WHERE source_table like 'amlakalahsa_%';
  IF v_n = 0 THEN
    RAISE EXCEPTION 'listing_native_location_v2 returns zero amlakalahsa rows — the view chain is broken';
  END IF;
END
$check$;
