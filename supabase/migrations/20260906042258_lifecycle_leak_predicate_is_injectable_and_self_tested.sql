...
begin
  ...
  if not public.ops_lifecycle_row_is_leaked(false, false, old, cut) then
    blind := array_append(blind, 'a genuine leak (absent from the matview, inactive, deactivated before a complete cycle) was NOT reported');
  end if;
  ...
  if not public.ops_lifecycle_row_is_leaked(false, null, null, cut) then
    blind := array_append(blind, 'a leaked row with a NULL deactivated_at was NOT reported');
  end if;
  ...
  if public.ops_lifecycle_row_is_leaked(false, true, null, cut) then
    blind := array_append(blind, 'a row that is still ACTIVE at source was reported as leaked');
  end if;
  if public.ops_lifecycle_row_is_leaked(true, false, old, cut) then
    blind := array_append(blind, 'a row still present in active_listing_ids_v2 was reported as leaked');
  end if;
  if public.ops_lifecycle_row_is_leaked(false, false, fresh, cut) then
    blind := array_append(blind, 'ordinary propagation latency (deactivated INSIDE the refresh+sync window) was reported as a leak');
  end if;
  -- Both null-cutoff shapes. The NULL-deactivated_at one is the load-bearing case: with a
  -- non-null stamp, dropping the cutoff guard yields `old < null` = NULL, which is falsy and would
  -- let the mutant survive. A legacy row with no stamp yields TRUE and catches it. (Found by
  -- running the mutation: the first version of this control passed against a predicate that had
  -- lost its blindness guard entirely.)
  if public.ops_lifecycle_row_is_leaked(false, false, old, null) then
    blind := array_append(blind, 'a stamped row was adjudicated as leaked while no complete cycle could be dated (must be blind, not clean)');
  end if;
  if public.ops_lifecycle_row_is_leaked(false, null, null, null) then
    blind := array_append(blind, 'an unstamped row was adjudicated as leaked while no complete cycle could be dated (must be blind, not clean)');
  end if;
  ...
  if position('ops_lifecycle_row_is_leaked(' in coalesce((
    ...
    blind := array_append(blind, 'ops_lifecycle_inactive_still_searchable() no longer calls the predicate this self-test proves');
  end if;
  ...
  if position('ops_lifecycle_inactive_still_searchable' in coalesce((
    ...
    blind := array_append(blind, 'mon_detect_inactive_still_searchable() no longer reads the candidate generator');
  end if;
