-- Wahadat joins listing_rich_attrs — the second Advanced-Filter view. Without an arm its listings
-- answer "unknown" to every rich-attribute question the AF can ask.
--
-- Arm shape is the wave-2 four's arm (20260926042448) verbatim, narrowed to two tables and appended
-- to the body read LIVE at apply time so another session's change is carried forward, not reverted.
--
-- Wahadat publishes almost none of these fields, and that is recorded honestly: everything the
-- source never states stays NULL rather than being defaulted to false. The lat/long CASE arms do
-- pick up real values, because the scraper stores the project's coordinates in additional_info.
--
-- Same short-lock retry loop as 20260926 wahadat_join_listing_extra_attrs: 3s lock_timeout per
-- attempt so live PostgREST readers are never parked behind this ACCESS EXCLUSIVE request, retried
-- into a natural gap, with statement_timeout widened only to cover the loop's own worst case.
...
BEGIN
  IF position('wahadat_residential_listings' in pg_get_viewdef('public.listing_rich_attrs'::regclass,true)) > 0 THEN
    RAISE NOTICE 'listing_rich_attrs already carries the wahadat arms';
    RETURN;
  END IF;
  FOR i IN 1..20 LOOP
    BEGIN
...
UNION ALL
 SELECT %1$L::text AS source_table,
  ...
  END IF;
  RAISE NOTICE 'listing_rich_attrs rebuilt with wahadat on attempt %', i;
END
$do$;
...
BEGIN
  IF position('wahadat_residential_listings' in pg_get_viewdef('public.listing_rich_attrs'::regclass,true)) = 0
     OR position('wahadat_commercial_listings' in pg_get_viewdef('public.listing_rich_attrs'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'wahadat did not land in listing_rich_attrs';
  END IF;
  IF position('tuba_residential_listings' in pg_get_viewdef('public.listing_rich_attrs'::regclass,true)) = 0
     OR position('ibaax_commercial_listings' in pg_get_viewdef('public.listing_rich_attrs'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'an existing platform arm was dropped while adding the wahadat ones';
  END IF;
  RAISE NOTICE 'wahadat is in listing_rich_attrs';
END $verify$;
