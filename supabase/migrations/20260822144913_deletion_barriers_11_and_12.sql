...
-- cleanup.py's final pre-delete recheck is the primary defense, but it is only as good as that
-- SAME code path at that SAME moment. This is a SECOND, independent check, run some time later
-- (scrapers/common/verify_deletions.py), against the same dead-marker registry — built to catch a
-- systematic bug in the recheck logic itself (a URL-construction error, a dead-marker regex that
-- stopped matching after a site redesign) that a single delete-time check cannot see.
create table if not exists public.cleanup_deletion_verification (
  id               bigint generated always as identity primary key,
  deletion_log_id  bigint      not null,   -- references cleanup_deletion_log.id (not an FK: the
                                            -- deletion_log row must survive independently of this
                                            -- table's retention, and cross-run integrity is exactly
                                            -- what mon_detect_cleanup_evidence_gap checks instead)
  ...
  http_status      int,
  verdict          text        not null,   -- 'dead' | 'live' | 'unknown', same vocabulary as verdict()
...
begin
  -- Each 'live' finding is a discrete, already-happened incident (a specific row was deleted and
  -- its own URL now serves live content again) — NOT an ongoing condition, so this never
  -- auto-resolves the way a condition-based detector would. It stays open until a human
  -- investigates and either confirms it was a legitimate later re-listing (a NEW ad reusing an old
  -- URL slug is possible on some platforms and is not the same claim as "we deleted a live row")
  -- or a genuine false-deletion the recheck logic itself must be fixed for.
  for rec in
...
-- The exact class of bug already found once this session (gathern_liveness_evidence_gap_barrier,
-- 2026-08-22): a table speced to hold per-row evidence for every consequential decision, silently
-- holding fewer rows than decisions made, because the writer had a gap nothing was watching. This
-- is the same barrier shape applied to cleanup_deletion_log specifically.
...
-- ── Roster registration, both detectors, SAME migration (AGENTS.md: a detector outside
-- mon_run_all_detectors is decoration, and mon_detect_orphaned_detectors fires on anything nothing
-- reaches). Needle-edit against the LIVE array — never a pasted full array.
do $roster$
declare
  src text;
...
-- ── Schedule the spot-check itself. Weekly, staggered AFTER all four cleanup runs (aqar 02:00,
-- wasalt 02:30, gathern 03:00, aqarcity 03:30 UTC Sunday) so there is something fresh in
-- cleanup_deletion_log to sample. workflow_dispatch inputs are all optional (empty platform ->
-- the workflow's own matrix checks all 4), so trigger_gh_workflow's no-inputs dispatch is
-- sufficient here — same pattern as gh-gathern-cleanup / gh-aqarcity-cleanup.
