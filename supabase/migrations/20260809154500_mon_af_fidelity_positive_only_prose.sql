...
-- exact premise that was wrong about aqar parking.
...
stats as (
  ...
  from search_listings_ar s cross join f
  where s.production_ready
  group by s.platform, f.field
)
select st.platform, st.field, st.rows_total, st.n_true, st.n_false, st.n_null,
