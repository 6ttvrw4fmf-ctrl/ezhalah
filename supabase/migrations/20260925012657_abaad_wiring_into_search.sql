-- أبعاد (abaad) joins search: its two tables become arms of listing_native_location_v1 and
-- active_listing_ids_v2. Until this runs, abaad_*_listings are tables no search can reach.
--
-- THIS IS THE 20260924170648 MACHINERY VERBATIM, with only the platform list and the presence
-- guards changed. That migration wired thirty-five platforms through the same steps earlier the
-- same day, so this is the proven path rather than fresh DDL surgery on the matview the whole
-- product reads: capture every dependent's definition/indexes/grants first, refuse to touch
-- anything if the probe returns zero dependents or an empty definition, build a `__mig` twin,
-- assert the twin still reads every source table the live body read, swap, then replay the
-- dependents. Any failed assertion raises and the whole transaction rolls back.
--
...
--
-- The post-swap loop checks abaad landed AND that all thirty-five arms wired earlier today are
...
DECLARE
  ...
  wire_v1 boolean := position('abaad_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0;
  wire_v2 boolean := position('abaad_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0;
BEGIN
  IF NOT wire_v1 AND NOT wire_v2 THEN
    RAISE NOTICE 'both matviews already carry abaad - nothing to do';
    RETURN;
  END IF;
  FOREACH t IN ARRAY tables LOOP
    IF to_regclass('public.' || t) IS NULL THEN
      RAISE EXCEPTION 'table % does not exist - apply abaad_listing_tables first', t;
    END IF;
  END LOOP;
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'abaad wired into search; all thirty-five earlier arms and every dependent intact';
END
$verify$;
