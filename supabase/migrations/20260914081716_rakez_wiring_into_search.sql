-- راكز العقارية JOINS SEARCH — owner-approved 2026-09-14.
-- Same capture-and-replay shape as 20260914070904 (suwar): the dependent views are read out of the
-- live catalog INSIDE this migration, a moment before the CASCADE, and replayed afterwards — so it
-- cannot restore a stale definition and keeps working as the dependency set grows.
DO $do$
DECLARE
...
BEGIN
  ...
  IF position('rakez_residential_listings.city_ar' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) > 0 THEN
  ...
  ELSE
    ...
    END IF;
    FOREACH t IN ARRAY ARRAY['rakez_residential_listings','rakez_commercial_listings'] LOOP
      arms := arms || format($f$
        UNION ALL
  ...
  arms := '';
  IF position('rakez_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) > 0 THEN
...
BEGIN
  IF position('rakez_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'rakez did not land in listing_native_location_v1';
  END IF;
  IF position('rakez_residential_listings' in pg_get_viewdef('public.active_listing_ids_v2'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'rakez did not land in active_listing_ids_v2';
  END IF;
  -- suwar must still be there: this migration CASCADEs the same matviews it wired an hour ago
  IF position('suwar_residential_listings' in pg_get_viewdef('public.listing_native_location_v1'::regclass,true)) = 0 THEN
    RAISE EXCEPTION 'suwar was lost while wiring rakez';
  END IF;
