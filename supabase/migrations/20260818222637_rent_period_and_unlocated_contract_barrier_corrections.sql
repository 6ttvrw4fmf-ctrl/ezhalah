-- Data Integrity run #29, owner product decisions (2026-08-18) - correcting the two barriers added
-- earlier in this session. Both corrections were found by RUNNING the detectors rather than trusting
-- that they were right, which is the §24e rule ("a fix whose premise was never verified is not a
-- fix") applied to barriers themselves.
--
-- CORRECTION 1 - mon_detect_unlocated_search_contract, limb (c). AS WRITTEN it fired on any
-- unlocated row whose city text matched ANY catalog city, and it duly raised P1 on 5 rows:
--
--   sanadak 585270 / 585133 / 1846796   «البريكه»  -> 3 catalog cities across 2 regions
--   sanadak 4368655                     «العقيق»   -> 4 catalog cities across 3 regions
--   aldarim 1565556                     «المجمعة»  -> 4 catalog cities across 3 regions
--
-- Not one of those sources publishes a region, so every one is the DUPLICATE SAUDI PLACE NAME case -
-- exactly the situation this run's headline fix established must resolve to NOTHING rather than to a
-- coin flip. The detector was firing on correct behaviour. A barrier that alarms on the rule it is
-- supposed to protect trains its readers to ignore it.
--
-- The owner's instruction when a barrier is wrong is not to silence it but to make it distinguish
-- cases. What limb (c) should assert is the RECOVERABLE case: an unlocated listing whose city text
-- resolves to EXACTLY ONE catalog city is a resolver failure - we had enough to place it and did
-- not. Ambiguous (>1) is honest UNKNOWN and must stay silent. Measured now: 0 unambiguous, 5
-- ambiguous, 65 with no catalog match at all. The limb is green for the right reason, and both
-- directions are mutation-proven.
--
-- CORRECTION 2 - mon_detect_rent_period_contract, payload. Limb 9 did `select ... into v`, clobbering
-- the payload accumulated by limbs 1-6, and the rebuild that followed dropped explicit_monthly_lost,
-- explicit_annual_lost, eastabha_listing_evidence_overridden and surfaces_not_reading_rent_period_ar.
-- `total` still counted them, so the detector fired correctly - but the alert a responder reads could
-- not say WHICH limb broke, and three of those are the owner's barriers 3, 4 and 8. Limb 9 now
-- accumulates into its own variable and every limb survives into the payload.
...
begin
  ...
  -- (9) owner barrier 9: every read surface must consume the CANONICAL field, never a raw scraper
  --     period. Accumulated into its OWN variable - an earlier revision selected into v and silently
  --     erased limbs 1-6 from the alert payload.
...
comment on function public.mon_detect_rent_period_contract() is
  'P1. The owner rent-period product decision of 2026-08-18, in nine limbs: sale never carries a '
  'period; explicit monthly and explicit annual both survive the fallback; rent always ends up with '
  'one; the monthly-only platforms never fall back to annual; eastabha stays per-listing; the '
  'fallback and its sale guard are still structurally present in the only writer of the canonical '
  'field; and every read surface still consumes rent_period_ar. 605 rows moved NULL -> سنوي when '
  'this landed, 0 -> شهري, 0 sale rows touched.';
...
declare
  ...
  struct jsonb := '[]'::jsonb; sample jsonb;
begin
  -- (a) an unlocated row must never carry city match ids - that is the branch that would let it
  --     into a city-filtered result set, i.e. contaminate a city/district count
  ...
  --     failure, not an honest unknown: we had enough to place it and did not. AMBIGUOUS names
  --     (>1 candidate city) are deliberately NOT counted - with no region published, picking one
  --     would be the duplicate-place-name bug this run fixed («البريكه» is 3 cities in 2 regions,
  --     «العقيق» 4 in 3, «المجمعة» 4 in 3). Unknown must stay unknown.
    ...
    n := public.mon_raise('P1', 'unlocated_search_contract', 'all', 'unlocated_search_contract',
      jsonb_build_object(
        ...
        'do_not', 'Do NOT make this green by giving a listing a city. Never invent a location. In '
                  'particular do NOT resolve an AMBIGUOUS city name by picking a candidate - this '
                  'limb deliberately ignores those, because 5 unlocated rows today carry names that '
                  'match 3-4 real Saudi cities each and no source region to choose between them.'));
  else
...
comment on function public.mon_detect_unlocated_search_contract() is
  ...
  'Unlocated rows must stay searchable with no location filter, must never carry match_city_ids or '
  'be production_ready (the two paths that would contaminate a city/district count), must never be '
  'handed a location we cannot prove, and the RPC must keep both the unlocated arm AND all three '
  'guards on it. Limb (c) counts only UNAMBIGUOUSLY resolvable city text - an earlier revision '
  'counted any catalog match and raised on 5 genuine duplicate-place-name rows, i.e. it alarmed on '
