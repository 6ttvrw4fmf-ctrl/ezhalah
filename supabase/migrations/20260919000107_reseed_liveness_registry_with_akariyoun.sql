-- عقاريون joins the liveness registry — a FULL RESEED (verify-liveness-registry-mirror.ts requires
-- the LATEST registry-seed migration to be the complete registry plus a delete clause; an
-- insert-only seed cannot retire a platform).
--
-- CRAWL_PRESENCE_ONLY, control-validated live 2026-09-18 against akariyoun's real retirement
-- behaviour: it hard-404s a url it no longer serves (this-slug-never-existed-zzz99 -> HTTP 404),
-- while two live listings answered HTTP 200 still rendering «رقم الاعلان». Unlike the WordPress
-- siblings there is no REST api to attribute a 404 to, so scrapers/akariyoun/run.py::_verify_gone
-- also requires the ad number on a 200 — a shell or a routing change must not read as alive.
-- Absence from the crawl only SELECTS candidates; _verify_gone gives each at-grace row a DIRECT
